...
**Bug B extension — execution_detail per-phase costs (2026-04-17):** The Bug B fix for `inv_cost` did not extend to `execution_detail.generation.cost` and `execution_detail.ranking.cost` inside `generateFromSeedArticle.ts` and `rankNewVariant.ts`. These still used `ctx.costTracker.getTotalSpent()` deltas (which delegate to the shared tracker on an `AgentCostScope`), inflating per-phase "actuals" 2-7x under parallel dispatch. This caused the Cost Estimates tab to show +136% estimation error when the real accuracy was ~3%. Fixed by replacing `getTotalSpent()` deltas with `getOwnSpent?.() ?? getTotalSpent()` in both files.
