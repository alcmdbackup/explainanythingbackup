...
**Bug B extension — execution_detail per-phase costs (resolved 2026-04-21):** Previously the Bug B fix for `inv_cost` did not extend to `execution_detail.generation.cost` and `execution_detail.ranking.cost` inside `generateFromSeedArticle.ts` / `rankNewVariant.ts` / `createSeedArticle.ts`. These used `ctx.costTracker.getTotalSpent()` deltas that inflated per-phase "actuals" under parallel dispatch. The 2026-04-20 refactor (`feat/investigate_under_budget_run_evolution_20260420`) extended the `getOwnSpent?.() ?? getTotalSpent()` fallback pattern to all four sites in `createSeedArticle.ts` (lines 89, 118, 133, 144) and verified the wiring against a clean staging run (2026-04-21 02:32 UTC). If per-invocation `cost_usd = 0` re-appears, `scope.getOwnSpent()` is returning 0 — inspect whether `ctx.rawProvider` is populated at the `Agent.run` call site and whether the scope is bound to the scoped `EvolutionLLMClient`.
