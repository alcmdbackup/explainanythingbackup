...
## High Level Summary
**CONFIRMED root cause:** the nightly `e2e-nightly.yml` runs `password-reset.spec.ts` against production; on the public host the guest auto-login session displaces the recovery session, so the form's `updateUser({ password })` overwrites the shared guest account instead of the test's dedicated `pwreset-e2e-*` user. This rewrites the guest DB password (the Vercel `GUEST_PASSWORD` env was stable for 5 days), breaking real-visitor autologin until manually reset. Confirmed via Supabase Management API auth logs: `user_modified` `PUT /user` on `guest@explainanything.app` at 2026-05-29 07:28:20 and 2026-05-28 07:28:44, each immediately preceding the first `invalid_credentials` failures. Ruled out: pg_cron (absent), Vercel cron (`crons: []`), edge functions (none), raw-SQL writers (`pg_stat_statements` shows only GoTrue writes the password). Fix shipped = form-level guest guard + `@skip-prod` on the spec + dedicated-user valid email (Option B). Preliminary leads below are retained as the investigation trail.
