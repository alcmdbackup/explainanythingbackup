...
- [ ] Add a new job to `.github/workflows/e2e-nightly.yml` (or a sibling scheduled workflow): build + start a **local** server with `TEST_LLM_MODEL=google/gemini-2.5-flash` and WITHOUT `E2E_TEST_MODE`, run only the `@prod-ai` grep, **chromium-only**. Uses staging Supabase + `OPENROUTER_API_KEY`. This job is allowed to soft-fail (see Rollback) so an OpenRouter outage doesn't red the whole pipeline.
- [ ] **Define a dedicated webServer/job for this tier** — do NOT reuse the port-3009 secondary server (it is gated on `RUN_GUEST_AUTO_TESTS=1` and exists for guest-autologin). Add a separate local-build invocation (own port) started with `TEST_LLM_MODEL` set + `E2E_TEST_MODE` unset, OR a standalone scheduled workflow that does `npm run build && TEST_LLM_MODEL=… npm start` then `playwright test --grep @prod-ai --project=chromium`.
- [ ] **Set retries for the `@prod-ai` local tier explicitly.** `playwright.config.ts:98` gives `retries: isProduction ? 3 : CI ? 2 : 0`; this tier runs locally (`isProduction=false`) so it would get CI's 2 — confirm that applies, and if the tier runs outside the `CI` flag, set `retries: 2` for the `@prod-ai` project so non-deterministic real-LLM output doesn't false-red. Retries must NOT mask a genuine pipeline break (assert on structural output, not exact text).
...
- [ ] Add a **GitHub Actions pre-flight step** (a workflow step BEFORE the `@prod-ai` playwright run) that reuses `LlmSpendingGate` / `daily_cost_rollups` to check the daily cap. If exhausted: **skip the `@prod-ai` job with a clear annotation** (not a hard red). Wire the `[release-health]` note as an **explicit follow-on step** (a skipped job emits no note by itself) — e.g. a `gh issue` append step conditioned on the pre-flight's skip output — so an exhausted account surfaces as an actionable note, not an opaque mid-run 429.
...
- [ ] Cheap real-AI tier (local build, real Gemini): `TEST_LLM_MODEL=google/gemini-2.5-flash npx playwright test --grep @prod-ai --project=chromium` (server started WITHOUT `E2E_TEST_MODE`; requires `OPENROUTER_API_KEY`).

## Documentation Updates
- [ ] `docs/docs_overall/environments.md` — **MUST fix in this PR** (factual error): line ~276 says nightly sets `E2E_TEST_MODE=true`; it does not (`testing_overview.md:437` and `e2e-nightly.yml` confirm "No E2E_TEST_MODE — uses real AI"). Replace with the correct statement and document the new local-build cheap-model `@prod-ai` tier + `TEST_LLM_MODEL` override + deployed-prod nightly trim.
