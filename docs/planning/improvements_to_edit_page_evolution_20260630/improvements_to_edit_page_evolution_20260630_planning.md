...
- [ ] Add `PUBLIC_EDIT_WIDEN_FILTER` env-var read in a shared helper `evolution/src/services/publicStrategyFilter.ts` (single source of truth for the filter shape used by both list + submit)
- [ ] Extract shared helper `assertStrategyPubliclySubmittable(strategy)` that both `listPublicStrategiesAction` and `submitPublicEditAction` call, keyed on `PUBLIC_EDIT_WIDEN_FILTER`
...
- [ ] Add Zod validation of `strategy.config.budgetUsd` in the submit path: `budgetUsd: z.number().positive()`. Reject with structured error `INVALID_STRATEGY_BUDGET` if malformed (defense-in-depth; the strategy row should never have a bad value but validate at the trust boundary)
...
- [ ] Refactor `EditForm.tsx:57-96` radio-card stack into the extended Combobox
  ...
  - [ ] **Empty-state branch:** when zero non-mock strategies returned, render the existing `edit-form-no-strategies` slot (preserve current empty-state UX). Combobox spec must cover this branch.
...
**Seed strategy fix (Task #3): choose option (b) — service-role inline seed in beforeAll.** Options considered:
- (a) `is_seed_smoke=true` bypass column — schema change; over-engineered
- **(b) service-role inline seed in beforeAll (chosen)** — zero-migration; hermetic per-spec; matches existing testing patterns; `afterAll` cleanup per Rule 16
- (c) route-mock the submit action — fragile; loses server-action coverage

With the mock-model filter (Q6) and widened filter (Q2), the currently-seeded `Public Edit Smoke` will (a) be excluded from the picker and (b) fail the whitelist re-check at submit. `edit-submit-flow.spec.ts` currently does `page.locator('[data-testid^="strategy-option-"]').first().click()` — after the widened filter this would click a REAL strategy and trigger real minicomputer LLM spend on CI. Fix: each E2E creates its own hermetic mock strategy inline.
...
- [ ] Modify `edit-submit-flow.spec.ts`:
  - [ ] `beforeAll`: service-role client inserts hermetic mock strategy with name `[E2E_INLINE] edit-submit-flow-<workerIndex>-<timestamp>` (auto-flagged `is_test_content=true` via trigger; deliberately test-content so it never leaks into any picker)
  - [ ] Test submits directly to the seeded strategy id via `submitPublicEditAction` mock (does NOT click picker's `.first()` — that would be a real strategy post-widening)
  - [ ] `afterAll`: cascade delete strategy + child runs (Rule 16)
  - [ ] Use per-worker `[E2E_INLINE]` name suffix per Rule 11 (avoid cross-shard collisions)
- [ ] **Deprecate `evolution/scripts/seedPublicEditE2EStrategy.ts`**: add DEPRECATED header comment. Remove its call site from any CI setup script. The mock strategy row it seeds becomes orphaned — one-time cleanup query in a follow-up (out of scope here).
...
- [ ] Add `src/__tests__/e2e/helpers/pages/EditRunPage.ts` extending `BasePage`
  ...
  - [ ] Actions returning `Locator` (NOT `Promise<string>`) per Rule 4 + ESLint `flakiness/no-point-in-time-pom-helpers`:
...
- [ ] Unit test `src/lib/utils/sanitizeMarkdownUrl.test.ts` (Task #6):
...
- [ ] **Rollback drill:** flip `PUBLIC_EDIT_WIDEN_FILTER=false` on staging; verify picker reverts to only `public_visible=true` strategies within 60s (cache TTL)
