...
## Phase 7 Staging A/B Results (2026-06-26 / 2026-06-27)

Phase 7 was executed as **three sequential staging A/Bs**, each addressing the open questions surfaced by the previous one. Full details in the planning doc's `## Artifacts` section. Quick summary:

1. **CoherencePassMode v1** (`3aad46c9`) — **INVALID**. Minicomputer ran pre-PR #1292 code. Quarantined.
2. **CoherencePassMode v2** (`efda7148`) — Mode B applies 100% of cycles vs Mode A's 17% — the PR #1292 structural fix works. But analysis surfaced confounds: parent Elo imbalance + child Elo collapse to a flat 1175–1220 band (TrueSkill under-measurement with `maxComparisonsPerVariant: 3`).
3. **CoherencePassEnabled** (`7ecb398a`) — the clean within-agent controlled experiment. Phase C enabled vs disabled with Phase A + B byte-identical. **Result: Phase C is approximately neutral on Elo at n=8/arm** (mean Δ −53.4 vs −53.1, Mann-Whitney p ≈ 0.56) and costs +5% per variant. **Full analysis**: [`docs/analysis/coherence-pass-enabled-ab-results-20260627`](../../analysis/coherence-pass-enabled-ab-results-20260627/coherence-pass-enabled-ab-results-20260627.md).

**Recommendation from Phase 7**: ship `coherencePassEnabled: false` as the default. The "−67 Elo coherence pass cost" prior observational finding doesn't replicate when Phase A + B are held constant.
