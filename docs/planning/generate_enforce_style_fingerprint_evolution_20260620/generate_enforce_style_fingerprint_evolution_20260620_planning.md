...
- [ ] `evolution/src/lib/pipeline/setup/renderFingerprintProse.test.ts` — deterministic prose from traits in BOTH scopes; `'article'` includes the cross-piece anti-overuse directive, `'paragraph'` OMITS it while keeping sentence-length/spelling/tone.
...
- [ ] `evolution/src/lib/core/agents/paragraphRecombine/buildParagraphRewritePrompt.test.ts` (extend) + `buildSequentialRewritePrompt` — paragraph-shaped `## Target Style` injected only when present; byte-identical when absent.
- [ ] `evolution/src/lib/shared/rubricJudge.test.ts` (extend) — `buildRubricComparisonPrompt` appends the target-style block for BOTH `mode:'article'` and `mode:'paragraph'` when `targetStyleProse` present; unchanged when absent.
- [ ] `evolution/src/lib/core/agents/paragraphRecombine/buildCoordinatorPrompt.test.ts` (extend) — coordinator prompt includes the paragraph style block when provided; byte-identical when absent (Phase 4 edits the coordinator, so it must not ship untested).
- [ ] Paragraph slot-judge integration/agent test (mock `slotLlm`) — asserts `ctx.styleFingerprint` snapshot → `perSlotConfig.targetStyleProse` (paragraph-shaped) → the `mode:'paragraph'` comparison prompt actually contains the paragraph prose. Closes the silent-drop guard end-to-end across BOTH the per-slot (`ParagraphRecombineAgent`) and sequential (`sequentialExecute`) paths.
...
- [ ] evolution/docs/rating_and_comparison.md — target-style context in the rubric judge (article + paragraph `mode`)
- [ ] evolution/docs/paragraph_recombine.md — paragraph-level style steering (rewrite + coordinator) and paragraph slot-judge target-style context
