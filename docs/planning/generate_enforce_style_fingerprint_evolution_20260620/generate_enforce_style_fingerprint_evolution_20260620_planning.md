...
**Status: READY FOR EXECUTION** — except for the scope addition below, pending targeted re-review.

### Post-consensus scope addition (2026-06-21): paragraph-level parity
After consensus, a design question surfaced that the reviewed plan only partially covered: the plan wired style into the **article-level** rubric/generation only. The `paragraph_recombine` path has its own paragraph-level rubric (`paragraphJudgeRubricId`) and rewrite generation. User decided **full parity** (judge + generate at paragraph level) with a **paragraph-shaped prose variant** (drops the cross-piece anti-overuse directive). Plan amended: Phase 3 (`renderFingerprintProse(traits, scope)`), Phase 4 (paragraph rewrite + coordinator injection), Phase 5 (paragraph slot-judge threading via `ParagraphRecombineAgent`/`sequentialExecute`), plus tests + `paragraph_recombine.md`. **This slice has NOT yet been through plan-review** — recommend a targeted re-review before executing the paragraph parts.
