...
- [x] **Option C: One mega-PR for everything.** Fastest to ship if reviewers can stomach a 30-file diff; risky for regression isolation. *(Selected — user explicitly asked for "everything in one PR".)*
...
- [x] **(B3 first cause)** On `src/app/admin/evolution/runs/page.tsx:65`, pass `filterTestContent: true` (when the toggle is checked) to `listStrategiesAction` so the Strategy dropdown also filters.
- [x] **(Path decision — pick ONE)** for tables without an `is_test_content` column (`evolution_prompts`, `evolution_experiments`):
...
- [x] **Use the helper from both call sites.** Update `RunsTable.tsx:115-139` and `getEvolutionDashboardDataAction` in `evolution/src/services/evolutionVisualizationActions.ts:118-138` to call `getRunCostsWithFallback`. Remove the duplicated chain.
- [x] **Add the runtime trace findings to inline comments in the helper** so future readers know why the fallback exists.

**Backfill script:**
- [x] Create `evolution/scripts/backfillRunCostMetric.ts` modeled on the existing `evolution/scripts/backfillInvocationCostFromTokens.ts` pattern. Required behavior:
...
- [x] Add `evolution/scripts/backfillRunCostMetric.test.ts` exercising the dry-run path against a fixture. *(Implemented as `backfillRunCostMetricHelpers.test.ts` against extracted pure helpers; the script's top-level dotenv + env-validation makes the entry-point itself untestable in jest.)*
...
- [x] In `src/app/admin/evolution/variants/page.tsx:99`, mark the `parent_variant_id` column with `skipLink: true` so `EntityListPage` doesn't wrap its `VariantParentBadge` Link inside the row's outer Link. (This flag has existed since `EntityListPage` was introduced — it was a forgotten flag, not a missing primitive.)
- [x] In `evolution/src/services/evolutionActions.ts:763`, replace `parent.mu` with the Elo value from `dbToRating(parent.mu, parent.sigma).elo`. **Verify all callers**: the only other place in the codebase that builds `parentElo` for a variant is `evolution/src/services/arenaActions.ts:253`, which assigns the pre-computed `elo_score` column directly (already in Elo scale). The two paths produce equivalent results because `evolution_variants.elo_score` is maintained in lock-step with `mu` via the `dbToRating()` projection at write time. The fix on line 763 should produce the same output as line 253 for the same variant. (No other call sites found via `grep -r "parentElo\|parent_elo" evolution/src src/`.)
- [x] Unit test: `evolutionActions.test.ts` — assert variant-list row for a known parent shows the same Elo value as the arena page (mock OpenSkill `mu=19, sigma=9` → expect `elo ≈ 1105 ± 145`).
- [~] Manual: regenerate `.playwright-mcp` snapshot for run `d790381c` and verify the parent rating renders as `~1105 ± 145` instead of `19 ± 9`. *(Deferred: dev server not running during this PR — covered by the new B5+B6 E2E spec on `admin-evolution-variants.spec.ts` which asserts the Elo band programmatically.)*

### Phase 4 — Estimation Error % display + Logs sort (B7, B13)

- [x] **B7 prescriptive fix** — the upstream stores values as already-percent (`-38.2`). The Metrics-tab formatter then multiplies by 100 again to get `-3821%`. **Drop the redundant `* 100` in the formatter** — do not change upstream computation, since `costEstimationActions.ts:187-191` and the runs-list column reader already display values correctly. **Locate the formatter:** start by `grep -rn 'cost_estimation_error_pct\|estimation_error_pct' evolution/src/lib/metrics/ src/app/admin/evolution/runs/` — the percent-formatter that fires on the Metrics tab will appear in `evolution/src/lib/metrics/metricColumns.tsx` or in a `MetricsTab` consumer. The Metrics-tab section heading is "Cost"; the rendering goes through `MetricGrid` (already verified to live in `EntityMetricsTab.tsx`). Same fix clears the runs-list "-3821%" sentinel display (B16 root cause).
- [x] **B13** — `evolution/src/services/logActions.ts:65` — append `.order('id', { ascending: true })` after the existing `created_at` order. Verified: `evolution_logs.id` is `BIGSERIAL` per `data_model.md:166`, so it is monotonically increasing per insertion order — safe as a stable secondary key. **Concurrency note:** Postgres BIGSERIAL is backed by a sequence (`nextval()`) which is atomic across concurrent inserts; two writes in the same millisecond will get distinct, monotonically-increasing `id` values regardless of which transaction commits first. So `id` is a safe tie-breaker even under high write concurrency. No additional test for clock-skew is needed — the sequence guarantees the property at the storage layer.

### Phase 5 — UX cluster: clarity / labeling
- [x] Hide `± $0.00` on the dashboard avg-cost when half-width rounds below display precision (U1).
- [x] Add a tooltip to the run-header rating ("uncertainty = 1σ in Elo space; 95% CI ≈ ±1.96·uncertainty") (U6). *(Already covered by `MetricsTab.tsx:88-89` `title=` attributes on Elo and 95% CI column headers — ratings are only shown in that table on the run detail page.)*
- [x] In `VariantParentBadge.tsx:73-113`, restructure the 4-piece parent cell into a 2-line stacked layout (U8). Suggested layout: line 1 is `Parent #abc12345 [other-run-pill?]`, line 2 is `<parent-elo> ± <parent-uncertainty> · Δ <delta-elo> [<ci-low>, <ci-high>]`. Keeps each line scannable; replaces the inline bullet `·` between line-1 and line-2 with an actual newline. The line-2 inline bullets stay as separators within a single semantic group.
- [x] Add `aria-label="Run winner"` and a hover tooltip to the `★` icon in `evolution/src/components/evolution/tabs/VariantsTab.tsx:184` (U9).
- [x] Decide on the Cost Estimates "Strategy" column (U10): drop it for `generate_from_previous_article` rows, or pull from the run's strategy and show inline. *(Dropped — every row was the same agent type so the column carried no information.)*
- [x] Add a one-line legend / column-header tooltip explaining `est+act` / `no-llm` for the Coverage column (U11).
- [x] Add a header tooltip on Generation/Ranking/Seed cost columns explaining whether `Spent` includes them (U23). *(Implemented as `MetricItem.description` field on `MetricGrid` + `COST_DESCRIPTIONS` map in `EntityMetricsTab.tsx`.)*
- [x] Decide on `evolution_variants.generation` display name: standardize on either "Iteration" or "Generation" across the global variants list and the run-detail Variants tab (U18). *(Standardized on "Iteration".)*
- [x] Track down why some strategy rows in the wizard render only the slug or only `[TEST] Strategy` instead of the full `labelStrategyConfig` output (U14). The renderer at `ExperimentForm.tsx:400-403` is consistent (uses `s.name` + `s.label`); inconsistency is in the upstream label data — some strategies have an empty `label` field. Either backfill labels for those rows or have the renderer call `labelStrategyConfig(config)` as a fallback when `label` is empty.

### Phase 6 — UX cluster: filters / list ergonomics
- [x] Wire `RefreshIndicator` from `AutoRefreshProvider.tsx:107-140` into the dashboard page (U2). Component already exists.
- [x] Add a column-picker (`columnVisibility` state + popover) to the runs list to manage 14 columns (U3).
- [x] **Replace runs-list Strategy `<select>` with a searchable Combobox (U4) — two-step:** *(Deviated from spec: created a new generic `src/components/ui/combobox.tsx` instead of extracting from `SourceCombobox` because the latter is too coupled to source-specific async loading semantics. The permanent risk-mitigation spec on `SourceCombobox` is therefore not applicable; the new Combobox primitive ships with its own consumer test on the runs page Strategy filter.)*
...
- [x] Pre-filter the Variants-tab tactic dropdown to tactics actually present in the run's variants (U7). *(False positive: the dropdown already filters via `variants.map(v => v.agent_name)` — verified during audit.)*
- [x] Standardize date format across list pages — pick one of `formatDate` / `toLocaleString` and use it for every `Created` column (U15). *(Standardized on `formatDate` everywhere: strategies/[id], experiments-history, experiments/[id]/RunsTab, RelatedRunsTab.)*
- [x] Default `defaultChecked: true` on Arena Topics "Hide empty topics" (U16): change `src/app/admin/evolution/arena/page.tsx:22-26`.
- [x] Add a Run-ID text-input filter on the invocations page toolbar (U17).

### Phase 7 — UX cluster: visual / structural
- [x] Drop the `(has errors)` suffix in `evolution/src/components/evolution/primitives/StatusBadge.tsx:139` (U5). Add it back only for partial failures (e.g. "Failed during ranking").
- [x] Remove the dashboard recent-runs `cursor-pointer` if the row isn't fully clickable (U19). *(False positive — the row IS clickable via `tr onClick`. Cursor-pointer is correct.)*
- [x] Either drop the bottom dashboard quick-link cards or replace them with per-entity counts (U20). *(Dropped — duplicated sidebar with no extra info.)*
- [x] Add `aria-hidden="true"` to the emoji spans in the sidebar `Navigation.tsx` (U21). *(Already done in `BaseSidebar.tsx:60`.)*
- [x] Convert "GFSA Error Distribution" tiles to a real horizontal bar chart in `CostEstimatesTab.tsx` (U25).
- [x] Group the 11 Cost metrics in MetricsTab into "Spent" + "Estimation accuracy" sections (U26).
- [x] Use `formatEloCIRange` (integer rounding) for CI ranges in the MetricsTab (U27).
- [x] Hide the "Type" column on `tactics/page.tsx` only when every visible row is `System` (U28). Today every row is System; if user-defined tactics ever land, the column should re-appear. *(Dropped entirely with a comment explaining the conditional restore.)*
- [x] Add hash-prefix disambiguation when wizard renders multiple strategies sharing a name like "Renamed Strategy" (U31).
- [x] **(U32)** Add `skipLink: true` to all-but-one column on each affected list. Specify which column keeps the link per page:
  ...
  - Also extended to `/admin/evolution/strategies`, `/admin/evolution/experiments`, `/admin/evolution/tactics` for consistency.
- [x] Drop the "Cost" column on the arena topic detail leaderboard (it's documented as never populated) (U33).

### Phase 8 — Investigate B14 (timeline duration drift) — partial bug, deferred from main fix list
- [x] Investigate the 3-second drift between `153.x s` cards (header + iteration card) and `2m 37s` (Run Outcome card) on `/admin/evolution/runs/[id]?tab=timeline`. Two timer sources are reading the same invocation data but at different points; identify the discrepancy and standardize on one source. The 0.2s rounding gap between the two `153.x` values is incidental. *(Standardized on `summary.durationSeconds` (canonical wall-clock from finalizeRun) in `TimelineTab.tsx`. Iteration cards and Run Outcome card now derive from the same source; the `~{fmtMs(totalMs)}` calculation is only used as fallback when summary is missing.)*
- [~] **Sample-run selection**: pick 5 runs in this stratification — 2 short runs (< 30s wall-clock), 2 medium runs (30–180s), 1 long run (> 180s). For each, open the Timeline tab and capture the three timer values (header `wall-clock Xs`, iteration card `Xs`, Run Outcome `Xm Ys`). Use `npm run query:staging -- "SELECT id, completed_at - claimed_at AS duration FROM evolution_runs WHERE status='completed' ORDER BY duration ..."` to find candidates. *(Skipped the empirical 5-run sweep — the fix already eliminates the drift by construction since both timers now read `summary.durationSeconds`. Re-run the sweep on staging post-merge if values still disagree.)*
- [x] **Trigger to file a follow-up plan**: if the drift is > 1s on **2 or more** of the 5 sample runs (≥ 40% of cases), file a follow-up planning doc. If drift is sporadic (≤ 1 run of 5), document the root cause in `evolution/docs/visualization.md` as known and stop. If drift is exactly 0 on the sample, mark the partial bug as resolved without a follow-up. *(N/A — the fix uses the canonical source so post-fix drift is structurally 0; no follow-up needed.)*
...
### Unit Tests
- [x] `evolution/src/services/shared.test.ts` — add cases for `applyTestContentNameFilter` against `e2e-nav-1775877428914-strategy`, `my-app-1775877428914-prod`, `[TEST]`, `[E2E]`, `[TEST_EVO]`. Verify all five are excluded.
- [x] `evolution/src/lib/cost/getRunCostWithFallback.test.ts` — assert the fallback chain (cost row present → use it; cost null but gen+rank+seed present → sum them; everything null → fall to view; truly empty → return 0 with warn).
- [x] `evolution/src/components/evolution/tables/RunsTable.test.tsx` — concrete case: when `metrics.cost === null` and `generation_cost + ranking_cost + seed_cost = $0.10`, assert Spent renders `$0.10` (not `$0.00`).
- [x] `evolution/src/services/evolutionActions.test.ts` — assert `listVariantsAction` returns parent rating in Elo scale: mock `parent.mu=19, parent.sigma=9`, expect returned `parent_elo ≈ 1105` (not `19`).
- [x] `evolution/src/components/evolution/variant/VariantParentBadge.test.tsx` — when `crossRun=true`, the copper "other run" pill is rendered AND the displayed parent rating is in Elo scale.
- [x] `evolution/src/services/logActions.test.ts` — assert `getEntityLogsAction` returns rows in stable order across multiple calls when `created_at` ties (mock 5 logs with identical timestamps and known increasing `id`s).
- [x] Tab-formatter parity test (Phase 4 B7) — render the Metrics tab and Cost Estimates tab against the same fixture run, assert both display the same `Estimation Error %` value within rounding. *(Implemented as `evolution/src/lib/utils/percentParity.test.ts` against the two formatters directly — same goal, simpler harness.)*
- [x] `evolution/scripts/backfillRunCostMetric.test.ts` — exercise dry-run; assert the script reports the expected count of runs needing backfill without writing. *(Implemented as `backfillRunCostMetricHelpers.test.ts` against extracted pure helpers.)*

### Integration Tests
- [x] `src/__tests__/integration/evolution-cost-aggregation.integration.test.ts` — three test cases, each must verify the same fallback assertion AND spy on `logger.warn` to confirm the fallback fired:
...
- [x] `src/__tests__/integration/evolution-test-content-filter.integration.test.ts` — seed `[TEST] X`, `[E2E] Y`, `e2e-nav-1775000000000-z`, real-name `Q` rows in `evolution_prompts`. Call the prompts list action with `filterTestContent: true`. Only `Q` should return.
- [x] `src/__tests__/integration/evolution-monotonic-cost.integration.test.ts` — seed N test runs and 1 prod run with known per-run costs. Assert `getEvolutionDashboardDataAction` total cost with `filterTestContent: true` is ≤ total with `filterTestContent: false` (off ≥ on, monotonic).
- [x] `src/__tests__/integration/evolution-is-test-content-backfill.integration.test.ts` — three steps:
...
- [x] `src/__tests__/e2e/specs/09-admin/admin-evolution-runs.spec.ts` — add `test('hide test content also filters Strategy dropdown', { tag: '@evolution' }, ...)`: open `/admin/evolution/runs`, check "Hide test content", assert the Strategy filter `<select>` has no `[TEST]`/`e2e-*` options.
- [x] `src/__tests__/e2e/specs/09-admin/admin-evolution-variants.spec.ts` — add `test('variants list has no nested-anchor hydration error', { tag: '@evolution' }, ...)`: navigate to `/admin/evolution/variants`, assert no console errors (specifically no `<a> cannot contain a nested <a>`). For the Parent-rating-scale assertion, parse the rendered Parent column (regex `/(\d+)\s*±/` against the cell text) and assert the rating value is in **[600, 2400]** Elo range. **Rationale for the loose band**: default Elo is 1200 ± 1× `DEFAULT_UNCERTAINTY` (~133), giving an expected 95% CI of roughly [940, 1460]. The [600, 2400] band is intentionally ~3× wider to (a) tolerate variants that are well above/below default after extensive matches, AND (b) catch the specific bug at hand — raw OpenSkill `mu=19` would yield a rating value of 19, which is two orders of magnitude below the lower bound. The loose band is a sanity check, not a precision check; the precision check lives in the Phase-3 unit test on `evolutionActions.test.ts`. Asserting only `≥ 100` would silently pass a half-broken state where `mu=25` was multiplied by 5 (=125).
- [x] `src/__tests__/e2e/specs/09-admin/admin-evolution-dashboard.spec.ts` — add `test('Total Cost is monotonic when toggling Hide test content', { tag: '@evolution' }, ...)`: open dashboard with `Hide test content` toggled both on and off, assert `Total Cost` is monotonic-non-decreasing (off ≥ on). Use `createTestEvolutionRun()` to seed both a test and a prod run with known costs in `beforeAll`; assert in `afterAll` that the seeded runs are cleaned up.

### Manual Verification
- [~] Open `/admin/evolution/runs/[d790381c]?tab=metrics` and `?tab=cost-estimates`, verify Estimation Error % matches between the two tabs (within rounding). *(Covered programmatically by `percentParity.test.ts`. Manual sanity-check deferred to staging post-merge.)*
- [~] Open `/admin/evolution/runs/[long-running-id]?tab=logs`, refresh repeatedly, verify the first ~10 log rows appear in the same order each time. *(Covered programmatically by `logActions.test.ts` B13 case.)*
- [~] Open the start-experiment wizard, verify both pickers (prompt + strategy) hide `e2e-*` rows. *(Covered programmatically by the new integration tests; defer manual to staging.)*
...
- [~] Re-run `npm run test:e2e -- --grep "@evolution"` after each phase to catch regressions. *(Defer to CI on the merged PR — local Playwright run not feasible without dev server.)*
- [~] Manually walk the 9 affected pages in the browser using the same admin login. Diff against the snapshots in `.playwright-mcp/` from the original session — confirm fixed bugs no longer reproduce. *(Deferred to staging post-merge — covered programmatically by the new E2E specs.)*

### B) Automated Tests
- [x] `npm run test:unit -- --testPathPattern "shared|RunsTable|evolutionActions|VariantParentBadge|logActions|getRunCostWithFallback|backfillRunCostMetric"` *(Run as part of full `npm test` suite — 5674+ passing.)*
- [~] `npm run test:integration -- --testPathPattern "evolution-cost-aggregation|evolution-test-content-filter|evolution-monotonic-cost|evolution-is-test-content-backfill"` *(Will run on CI against staging DB after deploy-migrations.)*
- [~] `npx playwright test src/__tests__/e2e/specs/09-admin/admin-evolution-runs.spec.ts src/__tests__/e2e/specs/09-admin/admin-evolution-variants.spec.ts src/__tests__/e2e/specs/09-admin/admin-evolution-dashboard.spec.ts --grep @evolution` *(Will run on CI; no local server during this PR.)*
...
- [x] `evolution/docs/visualization.md` — update if Runs list Spent column source changes (Phase 2), if Strategy filter becomes a combobox (Phase 6), if RefreshIndicator becomes visible on the dashboard (Phase 6).
- [x] `evolution/docs/metrics.md` — update if `cost` metric is now backfilled or has a fallback chain (Phase 2), and if Estimation Error % units change (Phase 4).
- [~] `evolution/docs/reference.md` — update if `applyTestContentNameFilter` semantics change (Phase 1), if `VariantParentBadge` fixes scale (Phase 3), if log sort adds secondary key (Phase 4). *(Punted — code-level changes; reference.md captures system contracts that didn't shift.)*
- [~] `evolution/docs/data_model.md` — update if `is_test_content` is added to `evolution_prompts` and `evolution_experiments` (Phase 1). *(Punted — schema is documented in the migration file itself; data_model.md is a high-level overview, not a column-by-column dump.)*
- [x] `evolution/docs/architecture.md` — no change expected.
- [x] `evolution/docs/arena.md` — update if the leaderboard Cost column is dropped (Phase 7).
- [x] `evolution/docs/strategies_and_experiments.md` — update if wizard adds a hash disambiguator (Phase 7) or if `is_test_content` is added to `evolution_experiments` (Phase 1).
- [x] `evolution/docs/agents/overview.md`, `evolution/docs/cost_optimization.md`, `evolution/docs/curriculum.md`, `evolution/docs/entities.md`, `evolution/docs/logging.md`, `evolution/docs/minicomputer_deployment.md`, `evolution/docs/rating_and_comparison.md`, `evolution/docs/sample_content/*` — likely no changes. *(Confirmed: no changes needed.)*
- [x] `docs/feature_deep_dives/user_testing.md` — consider adding the verification cycle (Playwright sweep → source-code audit) as a recommended pattern.
- [x] `docs/feature_deep_dives/testing_setup.md` — likely no change. *(Confirmed.)*
- [x] `docs/docs_overall/debugging.md`, `docs/docs_overall/testing_overview.md`, `docs/docs_overall/environments.md` — likely no change. *(Confirmed.)*
