...
- [x] Migration `supabase/migrations/<next-ts>_judge_eval_tables.sql` (idempotent, deny_all + service_role_all RLS, mirrors `20260524000003`):
...
- [x] `npm run lint:migrations` (idempotency lint — note it does NOT lint views, hence `CREATE OR REPLACE VIEW` above is mandatory) + `npm run db:types` to regen `src/lib/database.types.ts` (GENERATED `decisive` column lands read-only; Zod insert schema omits it).
- [x] Zod schemas in `evolution/src/lib/schemas.ts` (or a `judgeEval` schema module): `judgeEvalPairBankSchema` (incl. `pair_kind` enum), `judgeEvalTestSetSchema` (incl. `strategy` enum + per-kind sizes + seed), `judgeEvalRunSchema` (incl. `test_set_id` + `kind_filter`), `judgeEvalCallSchema` (incl. `pair_kind` + `comparison_mode`) — define the reasoning-effort enum inline as `z.enum(['none','low','medium','high'])` (mirrors the inline `debateJudgeReasoningEffort` at `schemas.ts:661,840` — there is NO exported shared symbol to reuse), `z.enum(['A','B','TIE'])` winners, `z.enum(['article','paragraph'])` kinds, confidence literal-union {0,0.3,0.5,0.7,1.0}.
...
- [x] `evolution/src/lib/judgeEval/persist.ts` — upsert run by settings_key (incl. kind_filter + **test_set_id**) for idempotent re-run; bulk-insert calls with denormalized `pair_kind` + `comparison_mode`; compute `prompt_variant_hash = sha256(mode + (customPrompt ?? builtin-template))`.

### Phase 3: CLI driver (+ seed command)
- [x] `evolution/scripts/judge-eval.ts` (`npx tsx`, dotenv + service-role client, mirrors `test-judge-models-v2.ts` + `debugProposerApproverFailures.ts` patterns).
...
### Unit Tests
- [x] `evolution/src/lib/judgeEval/metrics.test.ts` — decisive_rate/agreement/position_bias/accuracy/implied_beta formulas against fixed `CallResult` arrays (parity with `finalization.ts:83-86` + recovered `beta-analysis.ts`); metrics split correctly by `pair_kind`.
...
### B) Automated Tests
- [x] `npm test -- judgeEval`, `npm run test:integration:evolution`, then `npm run lint && npm run typecheck && npm run build`, then the E2E spec. Migration: `npm run lint:migrations` + `npm run migration:verify`.

### C) Rollback / blast-radius controls
- [x] **`JUDGE_EVAL_ENABLED` kill switch** (env, default `true`; `'false'` short-circuits `createEvalRunAction` + CLI sweep before any LLM call) — per-feature off-switch, since the global `LLMSpendingGate` is app-wide, not per-tool. String-contract `!== 'false'` (matches existing evolution flags).
- [x] **Hard cost ceiling** `JUDGE_EVAL_MAX_CALLS` / `JUDGE_EVAL_MAX_USD` (shared `assertWithinJudgeEvalCap` guard) rejects oversized sweeps server-side, independent of any user-supplied cap — bounds blast radius on the shared global budget (judge-eval is NOT per-user-capped).
- [x] Otherwise purely additive (new tables + new page + new actions + one CLI; reads existing `evolution_arena_comparisons` read-only at seed time; no migration to existing tables, no CI workflow change). Full rollback = revert the PR + drop the `judge_eval_*` tables. The `customPromptOverride`/`buildComparisonPrompt` shared code is unchanged (we call the existing 5-arg form, add no params).

## Documentation Updates
The following docs were identified as relevant and may need updates:
- [x] `evolution/docs/rating_and_comparison.md` — note the judge-eval tool reuses the comparison primitive; link historical methodology.
- [x] `evolution/docs/data_model.md` — add the `judge_eval_*` tables (pair_banks, test_sets, test_set_members, runs, calls) + leaderboard VIEW; note test-set freezing + comparability contract.
- [x] `evolution/docs/visualization.md` + `docs/feature_deep_dives/admin_panel.md` — document the Judge Lab page.
- [x] `evolution/docs/reference.md` — new files (`runJudgeEval.ts`, `judge-eval.ts`, `judgeEvalActions.ts`), CLI flags, env/cost notes.
- [x] `evolution/docs/metrics.md` — judge-eval metric definitions (decisive_rate parity, implied beta).
- [x] `evolution/docs/arena.md` — note judge_eval_* are separate from `evolution_arena_comparisons` (which stays the in-run match log).
- [x] `docs/research/judge_agreement_summary_tables.md` / `judging_accuracy_20260412.md` — cross-link the new repeatable tool.
- [x] New deep dive (optional): `docs/feature_deep_dives/judge_evaluation.md`.
