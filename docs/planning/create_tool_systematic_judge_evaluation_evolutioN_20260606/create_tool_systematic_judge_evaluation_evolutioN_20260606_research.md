...
### Federal Reserve 2 pair-bank — live data (staging dev DB, queried 2026-06-06)
User chose to source the bank from the **"Federal Reserve 2"** arena topic and to keep **article vs paragraph filterable separately** (first-class throughout).
- Article topic `evolution_prompts` row: **`a546b7e9-f066-403d-9589-f5e0d2c9fa4f`** name "Federal Reserve 2", prompt_kind=article (created 2026-04-15). (Distinct from the older "Federal Reserve" `50514a24…`, the source of the historical 140f7bce pairs.)
- **Article comparisons** (`evolution_arena_comparisons WHERE prompt_id=a546b7e9`): 7,096 rows, **7,005 distinct unordered pairs, 6,972 with both variant texts still present** (99.5%; entry FKs dropped so a few variants are gone), across 111 runs. Baseline judge (qwen-2.5-7b @ temp0) avg confidence **0.789**, **4,156 decisive (conf>0.6)**, **2,768 forced-tie (conf=0.5)** ≈ 40% — lots of close article pairs.
- **Paragraph comparisons** (slot-topics prompt_kind=paragraph, run_id ∈ FR2 runs): **1,889 distinct pairs, all with texts**, across 273 slot-topics / 19 runs. avg conf **0.754**, 1,014 decisive.
- **Total bank ≈ 8,861 pairs.** Storing all = cheap (DB rows + text snapshots). **Sweeping all = prohibitive**: 8,861 × (e.g. 8-cell grid) × 10 reps × 2 calls ≈ 1.4M LLM calls. → seed the FULL bank, but make the **sweep support kind-filter + sampling** (random or stratified by baseline-confidence bucket so each sweep gets a mix of decisive + forced-tie pairs). Recorded baseline confidence per pair becomes a free reference column.
- **Pair kind drives comparison_mode**: article pairs → `buildComparisonPrompt(mode='article')`; paragraph pairs → `mode='paragraph'` (TIE-discouraging rubric). The tool tags each pair `pair_kind ∈ {article,paragraph}` and auto-selects the matching mode; metrics + leaderboard are sliceable by kind.

Query recipe (read-only): article pairs = `prompt_id=a546b7e9`; paragraph pairs = `prompt_kind='paragraph' AND run_id IN (SELECT id FROM evolution_runs WHERE prompt_id=a546b7e9)`; join `evolution_variants` on entry_a/entry_b for `variant_content` + `mu`/`sigma` (Elo gap ground truth).
