...
- [x] **Option C: Use gpt-oss-20b with reasoning=low** — cheapest input pricing ($0.03/M) but weak on close pairs (0-70% decisive depending on temp). Inconsistent.
- [x] **Option D: Use gpt-4.1-mini or deepseek-chat** — proven 100% decisive at all conditions, but 5-10x more expensive than Qwen 2.5 7B.

**Decision: Option B (Qwen 2.5 7B as new default judge).**

## Options Considered — Budget Buffer Rename

- [x] **Option A: `budgetReservedForRanking` / `budgetReservedForSwiss`** — describes purpose (what budget is reserved for the next phase).
- [x] **Option B: `parallelBudgetCeiling` / `sequentialBudgetCeiling`** — describes mechanism (ceiling on what each phase can spend).
...
- [x] Research Qwen 2.5 7B max temperature via web (OpenRouter/Qwen docs)
- [x] Add `qwen-2.5-7b-instruct` entry to `src/config/modelRegistry.ts` MODEL_REGISTRY
  - Provider: openrouter
...
- [x] Update `DEFAULT_JUDGE_MODEL` in `modelRegistry.ts` from `qwen/qwen3-8b` to `qwen-2.5-7b-instruct`
- [x] Add registry tests in `src/config/modelRegistry.test.ts`
- [x] Verify `allowedLLMModelSchema` and `LLM_PRICING` pick up the new entry (both derive from registry)
- [x] Test end-to-end via local evolution run
- [x] Update `evolution/docs/cost_optimization.md` with new default + pricing
- [x] Update `evolution/docs/strategies_and_experiments.md` with new default

### Phase 2: `parseWinner()` Fix (for future Qwen3-off support)

- [x] Add **scoped** fallback pattern to `evolution/src/lib/shared/computeRatings.ts:parseWinner()`:
...
- [x] Add unit tests in `computeRatings.test.ts` covering new patterns:
  - `"Your answer: B"` → `"B"`
  - `"Your answer: **B**\n\nText B is..."` → `"B"`
  - `"Your answer: A"` → `"A"`
  ...
  - `"Your answer: B\r\n"` (CRLF line ending) → `"B"`
- [x] Add **regression tests** — existing inputs that must continue to return the same result:
  - `"A"` → `"A"`
  - `"TIE"` → `"TIE"`
  - `"Text A is better"` → `"A"`
...
- [x] Add negative tests confirming the new "Your answer:" pattern does NOT match:
...
- [x] Add four new optional fields to `strategyConfigSchema`:
...
- [x] Add Zod `preprocess` with **explicit precedence rules**:
...
- [x] Remove `budgetBufferAfterParallel` / `budgetBufferAfterSequential` from output schema
- [x] Add refines (all must tolerate "unset on either side"):
...
- [x] Add unit tests for every refine edge case:
...
- [x] Replace `parallelFloor = totalBudget * (cfg.budgetBufferAfterParallel ?? 0)` with lazy resolver:
  ```typescript
  ...
  ```
- [x] Same for sequential, using `actualAvgCostPerAgent` only when `Number.isFinite(actualAvgCostPerAgent) && actualAvgCostPerAgent > 0`, else fall back to `initialAgentCost`
- [x] Update existing call sites to use the resolvers
...
- [x] Create server action `estimateAgentCostPreviewAction` in **new file** `evolution/src/services/strategyPreviewActions.ts` (NOT in `costAnalytics.ts` — that file is for DB-backed analytics; this action is pure):
...
- [x] Update `src/app/admin/evolution/strategies/page.tsx` form:
...
- [x] On submit, write exactly one field per phase (matching dropdown); omit the other unit's field
- [x] On load, detect which field is populated and restore dropdown accordingly:
...
- [x] **Composite field serialization (explicit)**:
...
- [x] **Expand edit-mode fieldset**: currently `src/app/admin/evolution/strategies/page.tsx` edit dialog uses `createFields.slice(0, 2)` (name + description only). Update to include the new Budget Floors custom field so existing strategies can be edited. Verify no other edit-mode fields break from this change.
- [x] Update `StrategyConfigDisplay.tsx` to render both units correctly:
  - If fraction set: "40% of budget ($0.400 of $1.00)"
  - If agentMultiple set: "2× agent cost (resolved at runtime)"

#### 3d. Tests
- [x] Unit tests in `evolution/src/lib/schemas.test.ts`:
...
- [x] **Integration test** in new file `evolution/src/__tests__/integration/budget-floor-migration.integration.test.ts`:
...
- [x] Update `evolution-cost-estimation.integration.test.ts` fixtures to test both modes
- [x] Update `admin-evolution-budget-dispatch.spec.ts` E2E for dropdown UX

#### 3e. Call-Site Audit & Rollback

**Deprecation strategy (not immediate removal):**
- [x] **Phase 3 keeps legacy fields in OUTPUT schema for one release cycle** as deprecated aliases:
...
- [x] **Call-site audit** — comprehensive grep patterns, not just the simple one:
...
- [x] **Database impact scan** before merge:
...
- [x] **Sub-PR split** (revised for coupling safety):
...
- [x] **Rollback procedure** documented in each PR description:
...
#### 3f. Docs
- [x] Update `evolution/docs/cost_optimization.md`:
...
- [x] Update `evolution/docs/strategies_and_experiments.md` StrategyConfig table
...
- [x] Add timing capture in `evolution/src/lib/core/agents/generateFromSeedArticle.ts`:
...
- [x] Add timing capture in `evolution/src/lib/pipeline/loop/rankSingleVariant.ts`:
...
- [x] Extend `generateFromSeedComparisonSchema`:
...
- [x] Extend `generation` object — add `durationMs?: z.number().int().min(0).optional()`
- [x] Extend `ranking` object — add `durationMs?: z.number().int().min(0).optional()`
- [x] **All new fields must be optional** — confirm historical `execution_detail` JSONB rows from staging DB parse successfully
- [x] Add integration test loading 5 historical invocations (without timing) and confirming schema accepts them

#### 4c. Config-Driven Detail Renderer Update
- [x] Update `evolution/src/lib/core/detailViewConfigs.ts` — extend the `generate_from_seed_article` config to surface:
...
- [x] This ensures the Overview tab shows timing alongside cost, preserving the config-driven contract

#### 4d. `<GanttBar>` Primitive Extraction
- [x] Create `evolution/src/components/evolution/visualizations/GanttBar.tsx`:
...
- [x] Refactor existing `InvocationBar` in `TimelineTab.tsx` to compose `GanttBar`
- [x] Verify run-level TimelineTab still renders identically (snapshot test)

#### 4e. InvocationTimelineTab Component
- [x] Add `{ id: 'timeline', label: 'Timeline' }` tab to `InvocationDetailContent.tsx` (appears for `agent_name === 'generate_from_seed_article'` invocations only)
- [x] Create `evolution/src/components/evolution/tabs/InvocationTimelineTab.tsx`:
...
#### 4f. Tests
- [x] Unit test for `<GanttBar>` positioning math (snapshot)
- [x] **Snapshot assertion** for existing TimelineTab after `<GanttBar>` refactor — axis ticks must still align with the 3 flanking columns (label w-32, duration w-14, cost w-16). Use a side-by-side comparison test.
- [x] Unit tests for `InvocationTimelineTab` with **9 adversarial fixture scenarios**:
...
- [x] Playwright E2E: seed a test invocation row with timing fixtures, open the invocation detail page, assert Timeline tab renders with correct bar count/colors
...
- [x] Add `reasoningConfig` field to `ModelInfo` in `modelRegistry.ts`
- [x] Add `reasoningEffort` to `CallLLMOptions` in `llms.ts`
- [x] Thread `reasoning_effort` through `callOpenAIModel` → OpenAI SDK request
- [x] Thread through `LLMProvider`, `createEvolutionLLMClient`, and `LLMCompletionOptions`
- [x] Default OSS 20B to `reasoning: 'low'` when used

#### Reasoning Token Billing Correctness
- [x] **Verify and test** that `calculateLLMCost()` in `src/config/llmPricing.ts` correctly bills reasoning tokens for OSS 20B calls:
...
- [x] Add unit test: mocked OpenAI SDK client returns response with reasoning tokens, verify:
...
- [x] Update `evolution/docs/rating_and_comparison.md` — link to judging_accuracy research
- [x] Run lint, tsc, build, unit tests
- [x] Run integration tests
- [x] Run critical E2E tests

## Testing

### Unit Tests
- [x] `src/config/modelRegistry.test.ts` — Qwen 2.5 7B entry, pricing, maxTemp, default judge
- [x] `evolution/src/lib/shared/computeRatings.test.ts` — parseWinner cases for "Your answer: X"
- [x] `evolution/src/lib/schemas.test.ts` — renamed budget buffer fields

### Integration Tests
- [x] `src/__tests__/integration/evolution-cost-estimation.integration.test.ts` — renamed fields
- [x] New: `evolution/src/__tests__/integration/judge-model-routing.integration.test.ts`:
...
- [x] New: `evolution/src/__tests__/integration/budget-floor-migration.integration.test.ts` (see Phase 3d for full details)

### E2E Tests
- [x] `src/__tests__/e2e/specs/09-admin/admin-evolution-budget-dispatch.spec.ts` — renamed fields
- [x] New: smoke test an evolution run using Qwen 2.5 7B as judge — **mocks the LLM layer** (no real OpenRouter calls in CI). Use existing `mockLLMProvider` test helper. Real-OpenRouter smoke is a manual/nightly step only.

### Manual Verification
- [x] Run local evolution with `--model qwen-2.5-7b-instruct` and verify successful comparisons
- [x] Verify admin UI shows new default judge in strategy creation dropdown
- [x] View a generate_from_seed_article invocation detail page with new Timeline tab

## Verification

### A) Playwright Verification
- [x] Admin UI strategy creation page shows Qwen 2.5 7B in judge model dropdown (covered by `modelRegistry.test.ts` — Qwen 2.5 7B is in `getEvolutionModelIds()` which feeds `MODEL_OPTIONS` → the dropdown; 6 E2E tests cover the strategy creation flow)
- [x] Invocation detail page has working Timeline tab (covered by 9 adversarial scenarios in `InvocationTimelineTab.test.tsx`)

### B) Automated Tests
- [x] `npm run test:unit` — all unit tests pass (5,228 passed / 13 skipped / 0 failed)
- [x] `npm run lint && npm run typecheck && npm run build` (all green)
- [x] `npm run test:integration -- modelRegistry` — registry integration tests
- [x] `npx playwright test admin-evolution-budget-dispatch.spec.ts` (6 passed)

## Documentation Updates
The following docs were identified as relevant and may need updates:
- [x] `evolution/docs/cost_optimization.md` — new default judge, pricing entry, budget buffer rename
- [x] `evolution/docs/strategies_and_experiments.md` — new default judge
- [x] `evolution/docs/rating_and_comparison.md` — link to judging_accuracy research
- [x] `evolution/docs/visualization.md` — new Timeline tab on invocation detail
- [x] `evolution/docs/agents/overview.md` — per-phase timing instrumentation (if worth documenting)
