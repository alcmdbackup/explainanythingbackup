...
- [ ] `evolution/src/lib/weightInference/autoJudge.test.ts` (auto mode) — injected mock `JudgeFn` → deterministic verdicts → known recovered weights; `auto_repeats>1` folds K runs into one canonical verdict + cross-repeat-agreement confidence; canonical orientation correct.
- [ ] `evolution/src/lib/weightInference/cap.test.ts` (auto mode) — `assertWithinWeightInferenceAutoCap`: `plannedCalls = remainingPairs × repeats × 4` boundary just under/over `MAX_CALLS` + `MAX_USD`; `WEIGHT_INFERENCE_AUTO_ENABLED='false'` rejects; defaults (8000 / $5).
