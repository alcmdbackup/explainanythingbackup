...
- **Kill-switch CI wiring:** add `EVOLUTION_WEIGHT_INFERENCE_ENABLED` to the `integration-evolution` job env block in `.github/workflows/ci.yml` (default-on), mirroring how `EDITING_AGENTS_ENABLED` / `EVOLUTION_DEBATE_ENABLED` are wired, so the default-on path is exercised and the flag is greppable. Convention: `process.env.EVOLUTION_WEIGHT_INFERENCE_ENABLED !== 'false'` (default-on; only the exact string `'false'` disables).
