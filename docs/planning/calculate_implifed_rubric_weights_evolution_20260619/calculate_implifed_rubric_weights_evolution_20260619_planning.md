...
- `evolution_weight_inference_comparisons` — one row per human-judged pair: `id`, `session_id CASCADE`, `article_a_id→articles`, `article_b_id→articles`, `overall_winner TEXT CHECK ('a','b','tie') NULL` (NULL until the overall step is done), `rater_id`, `created_at`, `updated_at`; UNIQUE `(session_id, article_a_id, article_b_id, rater_id)` storing canonical (min,max) order to dedupe. (Mirrors `evolution_arena_comparisons`.)
...
- `fitWeights(comparisonsWithVerdicts, criteriaIds, opts)` → `{ weights:[{criteriaId,weight}] (non-neg, sum-1), logLik, trainAccuracy, heldOutAccuracy?, collinearity, perCriterionAgreement, perWeightCI }`. Non-negative logistic/BT (softmax-param or clamp-refit) with L2 reg on the verdict vectors `vᵢ∈{−1,0,+1}`; sigmoid per `swissPairing.ts`, logit clamp per `judgeEval/metrics.ts`. Only pairs with BOTH the overall verdict AND a complete per-criterion verdict set enter X; overall-ties dropped (noted). A criterion whose verdicts never align with the overall outcome ⇒ near-0 weight ("barely matters"); a criterion that systematically *opposes* the overall ⇒ would-be-negative ⇒ clamped + flagged ("disagrees with overall").
...
- `requiredRatings(K, targets)` (upfront rule-of-thumb: pairs ≥ ~10–15×K) and `estimateRemaining(currentData, targets)` (live, simulation-backed) + collinearity/identifiability detection (criteria whose verdicts move together can't be separated → warn).
...
- `recordDimensionVerdictsAction({sessionId, articleAId, articleBId, verdicts:[{criteriaId, verdict}]})` and `recordOverallVerdictAction({sessionId, articleAId, articleBId, overallWinner})` — independent per-step writes against the same comparison row (upsert; create the comparison row on first write for the pair).
- `getNextPairAction({sessionId, step})` — pick the next pair to judge for the per-criterion or overall step (prioritize uncovered/informative pairs; randomize A/B orientation).
...
- [ ] Topic→article seeding (snapshot variants), per-step verdict upserts (comparison + dimension-verdict rows), fit-on-read, export-to-rubric (`createJudgeRubricAction`).
