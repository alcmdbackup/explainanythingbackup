...
- [ ] Wire `WEIGHT_INFERENCE_AUTO_ENABLED='true'` + `WEIGHT_INFERENCE_AUTO_MAX_CALLS` + `WEIGHT_INFERENCE_AUTO_MAX_USD` + `WEIGHT_INFERENCE_AUTO_CHUNK_PAIRS` into BOTH the `integration-evolution` AND `e2e-evolution` env blocks in `.github/workflows/ci.yml` (e2e-evolution currently has no feature-flag env).
