...
- [x] **Option B: Type-system-first** — Start with branded types and discriminated unions. Higher upfront refactor cost.
- [x] **Option C: Full formal methods** — TLA+ models + branded types + property tests. Maximum coverage but high effort.
...
- [x] Install `fast-check@^3` as devDependency: `npm install --save-dev fast-check@^3` (pin major version to avoid breaking API changes between v2→v3)
- [x] Create `evolution/src/lib/shared/selectWinner.ts` — extract unified winner determination function
...
- [x] Create `evolution/src/lib/shared/selectWinner.test.ts` — unit tests for extracted function
  - Highest mu wins, sigma tiebreak, unrated handling, empty pool throws, all-unrated fallback
- [x] Export `selectWinner` from barrel file `evolution/src/lib/index.ts` to follow existing public API convention
- [x] Update `evolution/src/lib/pipeline/loop/runIterationLoop.ts` — replace inline winner logic with `selectWinner()`
...
- [x] Update `evolution/src/lib/pipeline/finalize/persistRunResults.ts` — replace inline winner logic with `selectWinner()` (no behavioral change — already uses `?? -Infinity` semantics)
- [x] Create `evolution/src/lib/shared/computeRatings.property.test.ts` — property-based tests for rating math
...
- [x] Create `evolution/src/lib/pipeline/infra/trackBudget.property.test.ts` — property-based tests for budget
...
- [x] Run lint, tsc, build, all evolution unit tests — verify no regressions
...
- [x] Add `safeParse` to RPC claim response in `evolution/src/lib/pipeline/claimAndExecuteRun.ts` (~line 112) — replace `as unknown as ClaimedRun[]` with Zod schema validation. On parse failure: log error with raw response shape, return `{ claimed: false }` (same as "no pending runs" path — pipeline-resilient, no crash).
- [x] Add `Number.isFinite()` checks on arena entry mu/sigma in `evolution/src/lib/pipeline/setup/buildRunContext.ts` (loadArenaEntries) — NaN is not caught by `??` (Risk #3)
- [x] Add null validation on `resolveContent` return in `evolution/src/lib/pipeline/setup/buildRunContext.ts` (resolveContent) — replace `as string` cast (Risk #4)
- [x] Add `validateFormat()` call to seed article content in `generateSeedArticle.ts` — currently the only LLM text path that skips format validation
- [x] Upgrade Agent detail validation from warn-only in `evolution/src/lib/core/Agent.ts` (~line 36): skip writing `execution_detail` to DB on parse failure (log warning, set `execution_detail: null`), but do NOT throw — preserve pipeline resilience while preventing corrupt detail from reaching DB (Risk #5)
- [x] Add `Number.isFinite()` guard to `writeMetric()` in `evolution/src/lib/metrics/writeMetrics.ts` — reject NaN/Infinity before DB write (Risk #8)
- [x] Add precondition to `createCostTracker()` in `evolution/src/lib/pipeline/infra/trackBudget.ts`: `if (!Number.isFinite(budgetUsd) || budgetUsd <= 0) throw` — rejects NaN, Infinity, negative, and zero (Risk #7)
- [x] Audit and fix all `?? DEFAULT` fallbacks on numeric DB fields — replace `??` with `Number.isFinite()` guard:
...
- [x] Create migration `supabase/migrations/20260328000001_add_evolution_constraints.sql` (follows existing `YYYYMMDDNNNNNN` naming scheme):
...
- [x] Run `npm run db:types` after migration to regenerate `src/lib/database.types.ts` reflecting new constraints
- [x] Deploy to staging first, verify no constraint violations
- [x] Run E2E tests to confirm pipeline still works end-to-end with new constraints

### Phase 4: Format Validator Property Tests + Budget Assertions (~0.5 day)

- [x] Create `evolution/src/lib/shared/enforceVariantFormat.property.test.ts`
...
- [x] Add budget postcondition assertions to `evolution/src/lib/pipeline/infra/trackBudget.ts`
...
### Unit Tests
- [x] `evolution/src/lib/shared/selectWinner.test.ts` — 5+ cases: highest mu, sigma tiebreak, unrated, all-unrated, empty pool
- [x] `evolution/src/lib/shared/computeRatings.property.test.ts` — 10 property-based tests for rating math + comparison (with `jest.unmock('openskill')`)
- [x] `evolution/src/lib/pipeline/infra/trackBudget.property.test.ts` — 6 property-based tests for budget invariants
- [x] `evolution/src/lib/shared/enforceVariantFormat.property.test.ts` — 8 property-based tests for format validation
- [x] Phase 2 negative-path tests: safeParse failure, NaN rejection, empty LLM response, invalid Agent detail, negative budget

### Integration Tests
- [x] Existing evolution pipeline tests pass with `selectWinner()` extraction (no new integration tests needed)

### E2E Tests
- [x] Existing E2E specs (`admin-evolution-run-pipeline.spec.ts`) pass — pipeline still produces correct winners
- [x] No new E2E specs needed (this is infrastructure/testing work, not user-facing)

### Manual Verification
- [x] Run `npm run test` in evolution and confirm all 1,145+ tests pass
- [x] Run DB constraint audit queries on staging before migration
- [x] Run DB migration on staging, verify no constraint violations
- [x] Run one evolution pipeline execution end-to-end after changes
...
### B) Automated Tests
- [x] `npx jest evolution/src/lib/shared/selectWinner.test.ts` — winner extraction tests
- [x] `npx jest evolution/src/lib/shared/computeRatings.property.test.ts` — rating property tests
- [x] `npx jest evolution/src/lib/pipeline/infra/trackBudget.property.test.ts` — budget property tests
- [x] `npx jest evolution/src/lib/shared/enforceVariantFormat.property.test.ts` — format property tests
- [x] `npm run test` — full test suite
- [x] `npm run tsc` — type checking
- [x] `npm run lint` — lint pass
- [x] `npm run build` — build pass

## Documentation Updates
The following docs were identified as relevant and may need updates:
- [x] `evolution/docs/reference.md` — add selectWinner to key files table, mention fast-check in testing section
- [x] `evolution/docs/rating_and_comparison.md` — document rating invariants proven by property tests
- [x] `evolution/docs/cost_optimization.md` — document budget postcondition assertions
- [x] `evolution/docs/data_model.md` — document new CHECK constraints and UNIQUE on config_hash
