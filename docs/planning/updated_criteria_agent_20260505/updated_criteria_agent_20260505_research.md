...
1. **Single-pass tactic name** — RESOLVED 2026-05-06: Option B. Two new marker tactics `'criteria_driven_single_pass'` and `'criteria_driven_propose_approve'`. GFPA's misconfiguration guard widens to accept all three (`'criteria_driven'`, `'criteria_driven_single_pass'`, `'criteria_driven_propose_approve'`). Both new tactics registered in `MARKER_TACTICS` (`evolution/src/lib/core/tactics/index.ts`); `syncSystemTactics.ts` unions them into the DB so they appear on the tactic leaderboard for direct A/B comparison alongside the legacy `'criteria_driven'`.

2. **`mirrorEdits.ts` location** — RESOLVED 2026-05-06: Option A. File lives at `evolution/src/lib/core/agents/editing/mirrorEdits.ts` alongside the existing CriticMarkup primitives (`parseProposedEdits.ts`, `applyAcceptedGroups.ts`, `validateEditGroups.ts`). Treats the mirror helpers as CriticMarkup primitives first, propose/approve specifics second. Keeps the dependency graph readable and leaves the door open for reuse by future agents that operate on edit groups.

3. **`mirrorAgreementRate` alert thresholds** — RESOLVED 2026-05-06: Option B (two-sided). Two env vars:
   - `EVOLUTION_PROPOSER_APPROVER_CRITERIA_MIRROR_AGREEMENT_LOW_THRESHOLD` (default `'0.20'`) — alert when rate falls BELOW (low yield / approver-bias signal).
   - `EVOLUTION_PROPOSER_APPROVER_CRITERIA_MIRROR_AGREEMENT_HIGH_THRESHOLD` (default `'0.95'`) — alert when rate exceeds (rubber-stamp signal).
   Run-detail dashboard alert-colors out-of-band values red, mirroring `iterative_edit_*_ALERT_THRESHOLD` pattern. Defer adding a separate `mirror_approver_acceptance_rate` metric until real data shows whether `mirrorAgreementRate` alone is sufficient.

4. **Per-iteration `includesMirrorApprover` field vs env-var kill switch** — RESOLVED 2026-05-06: Option B (config field only). `IterationConfig.includesMirrorApprover?: boolean` (default `true`) — set in the strategy wizard, persisted on the strategy, participates in `config_hash`. **No** `EVOLUTION_PROPOSER_APPROVER_CRITERIA_MIRROR_ENABLED` env var. Drop that flag from the kill-switch table; if mirror is buggy, ops route is to disable the whole `EVOLUTION_PROPOSER_APPROVER_CRITERIA_ENABLED` flag and fix the code, not flip mirror-only behavior. **Updated kill-switch count: 6 new env vars (down from 7).**

5. **Approver `cycle` vs `cycles[]`** — RESOLVED 2026-05-06: Option B. Use `cycles: [{ ... }]` (array with length always 1) to match `IterativeEditingAgent`'s shape verbatim. Detail-view-config entries can be near-copies of the editing agent's (`cycles.0.proposedGroupsRaw`, `cycles.0.forwardDecisions`, etc.). Zod refinement enforces `cycles.length === 1` at validation time so the array contract is documented but constrained. Schema additions to `execution_detail` use `cycles[0].forwardDecisions`, `cycles[0].mirrorDecisions`, `cycles[0].appliedGroups`, `cycles[0].proposeCostUsd`, `cycles[0].approveForwardCostUsd`, `cycles[0].approveMirrorCostUsd`, etc.
