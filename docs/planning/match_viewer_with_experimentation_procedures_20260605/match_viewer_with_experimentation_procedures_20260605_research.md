...
- **Reasoning output needs a reasoning-tolerant parser + raw-trace capture.** `parseWinner` (`computeRatings.ts:380`) is built for a single-token reply: it does a bare `contains 'TIE'|'DRAW'|'EQUAL'` check (so a reasoning paragraph saying "both are equally clear" → false TIE) and its `Your answer:` regex is anchored to the **start** (`^\s*YOUR ANSWER`), so a verdict that trails reasoning won't match. The sandbox's "Explain reasoning" mode therefore (a) instructs a strict FINAL verdict line and (b) parses the **last** verdict marker via a dedicated scan, not `parseWinner`. `compareWithBiasMitigation` also returns only `{winner, confidence, turns}` — it does not expose prompts/raw responses — so the re-judge action drives `run2PassReversal` directly and captures `prompt` + `rawResponse` per pass for display.
