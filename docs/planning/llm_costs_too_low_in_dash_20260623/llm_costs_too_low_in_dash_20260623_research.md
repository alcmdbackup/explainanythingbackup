...
- **Refined conclusion (then CORRECTED — see below):** initially concluded the *runner* never invokes `saveTrackingAndNotify`.

- **FINAL ROOT CAUSE (2026-06-23, code-verified — supersedes all above):** The **production runner path is CORRECT.** `claimAndExecuteRun.ts:204-223` → `callLLM` with `requireTracking:true`; `createEvolutionLLMClient.ts:206` calls that same `rawProvider.complete`, preserving `requireTracking` end-to-end. The runner journal shows `Claimed evolution run` + `Function callLLM completed successfully`; the ~49 evolution tracking rows/30d are from here. The runner is mostly idle and the claim-gate makes it skip `[TEST]` strategies. **The real bypass is the DIRECT-PROVIDER dev/experiment path:** `evolution/scripts/run-evolution-local.ts` builds `createDirectLLMProvider` (line 190) calling the SDK directly + costing via `calculateLLMCost`, **never touching `callLLM`/`saveTrackingAndNotify`** (which is why it's in the `check-llm-call-coverage` ALLOWLIST). That yields real-LLM-output invocations with `cost_usd` + `status=completed` + 0 tracking rows — matching the 733 real + thousands of test trackingless invocations. Evidence: test invocations' `execution_detail` contains real generated article text (deepseek-v4-flash/gpt-4.1-nano); real (non-test) 7d = 733 invocations / 0 tracking rows.
- **Correction note:** earlier in this session I asserted (a) "stale runner," (b) "wrong-target writes," (c) "runner bypasses saveTrackingAndNotify." All three were wrong. The verified answer is: prod runner tracks correctly; direct-provider dev scripts bypass tracking by design.
