...
- **The anomaly:** under fail-closed wiring, `success=true` + 0 staging tracking rows + 0 dead-letters should be impossible IF calls route through `saveTrackingAndNotify`. **Leading hypothesis (UNVERIFIED — needs a read-only prod query, currently out of the dev-only scope):** staging-claimed runs' tracking writes are landing on the **prod** client (the runner holds both), so the rows exist on prod, not staging. Alternatives: the LLM call path in the runner doesn't invoke `saveTrackingAndNotify`, or a per-target client mix-up.
