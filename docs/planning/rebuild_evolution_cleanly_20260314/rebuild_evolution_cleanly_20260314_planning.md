...
- `evolution/src/lib/v2/runner.ts` (~250 LOC) — Core run execution module:
  - **Logging**: Use `console.warn`/`console.error` for runner logging (no external logger dependency). V2 keeps logging minimal — errors go to stderr, structured run metadata (runId, status) logged on start/complete/fail.
  ...
  - Heartbeat (30s interval via setInterval, cleared in finally block; `clearInterval(undefined)` is a no-op in Node.js so null-guarding is implicit). Heartbeat DB error is non-fatal (try/catch with console.warn inside the interval callback).
  ...
  - **Result persistence (minimal, before M5)**: After evolveArticle returns EvolutionResult, persist winner variant to `evolution_variants` with explicit columns: `(id, run_id, variant_content, elo_score, generation, parent_variant_id, agent_name, match_count, is_winner)` where `id = crypto.randomUUID()`, `elo_score = toEloScale(winnerRating.mu)`, `generation = winner.iterationBorn`, `parent_variant_id = winner.parentId ?? null` (from EvolutionResult's winner metadata), `agent_name = winner.strategy`, `match_count = winner.matchCount` (total comparisons the winner participated in), `is_winner = true`. Mark run as `status: 'completed'` with `run_summary: { version: 3, totalIterations, stopReason, totalCost }`. **Note**: seed generation cost is untracked by cost tracker (pre-pipeline, ~$0.02 for 2 LLM calls) — totalCost in run_summary reflects pipeline costs only, matching V1 behavior. M5's finalize.ts will replace this with full V1-compatible persistence (all pool variants, detailed run_summary, experiment auto-completion). This avoids circular dependency with M5.
...
- `evolution/scripts/evolution-runner-v2.ts` (~100 LOC) — CLI batch entry point (separate from module):
  ...
  - **Runner identity**: Generate `runnerId` as `v2-${hostname()}-${process.pid}-${Date.now()}` (unique per process, used for claim RPC and heartbeat identification). Supabase service client created via `createSupabaseServiceClient()` from `src/lib/supabase/server.ts`.
...
- `evolution/src/lib/v2/index.ts` — Add M4 exports (`executeV2Run`, `generateSeedArticle`); add re-exports of V1 modules: `rating` (Rating type, createRating, toEloScale), `comparison` (compareWithBiasMitigation), `formatValidator` (validateFormat), `textVariationFactory` (createTextVariation); re-export `evolveArticle` if not already from M3
...
**Deploy after**: Milestone 9 (M9 drops `continuation_pending` from claim RPC — required to avoid V2 claiming V1 continuation runs)
...
- `evolution/src/lib/v2/finalize.ts` (~190 LOC) — Persist V2 results in V1-compatible format
  ...
  - **Strategy aggregate update**: After variant upsert, if `run.strategy_config_id` is set, call `update_strategy_aggregates` RPC with `p_strategy_id = run.strategy_config_id`, `p_cost_usd = result.totalCostUsd ?? 0` (total LLM cost from pipeline), and `p_final_elo = toEloScale(winnerMu)` (deferred from M4 which lacks final Elo). The RPC parameter name is `p_strategy_id` (NOT `p_strategy_config_id`) — matches V1's `metricsWriter.ts:28`. **M1 prerequisite**: EvolutionResult MUST include `totalCostUsd: number` (or finalize.ts accepts it as a separate parameter). If strategy_config_id is null, skip (no-op). Failure is non-fatal — log warning and continue.
...
- **finalize.test.ts** (~300 LOC, 21 tests):
  ...
  - (19) Strategy aggregate update: update_strategy_aggregates RPC called with correct `p_strategy_id`, `p_cost_usd`, and `p_final_elo` after variant upsert; null strategy_config_id skips call
...
**Table notes**:
- The ~41,710 LOC total from research was approximate. Row-level verified sums total ~43,943 Before. The ~2.2K gap is due to double-counting: page tests (2,517 LOC) partially overlap with component tests already counted in the component keep/delete/shrink rows (which total 3,994 LOC and include some page-level component tests). The true deduplicated Before total is ~41,710; the After total adjusts proportionally to ~13,700. The reduction percentage (~67-68%) holds either way. The 1,514 LOC of service tests kept until M7/M10/M11 are included in the After total since they are actively kept in M8.
...
**Integration test infrastructure**: V2 integration tests use `supabase start` (local Supabase). **DB schema dependency**: V2 integration tests run against the existing V1 schema tables (runs, variants, invocations) which V2 shares — no new M9 migration required for basic lifecycle tests. If M9 adds new columns/tables that specific V2 integration test cases need, those cases are deferred to after M9. V2 unit tests (which mock all DB calls) have no schema dependency at all. Test setup: `beforeAll` creates a test run + topic via service_role key, `afterAll` deletes test data. LLM is mocked via Jest mock (not real API calls). This matches V1's existing integration test pattern (see `evolution-infrastructure.integration.test.ts`). CI runs integration tests via `test:integration:evolution` script which expects local Supabase to be running (started by CI workflow `supabase start` step).
...
**PR sequencing**: PRs 2-6 (deletion PRs) must be merged sequentially — each PR requires CI green (all tests pass, `tsc --noEmit` clean, coverage >= 40% floor) before the next PR is opened. PRs 1, 7, and 8 have no ordering constraints relative to each other beyond their stated dependencies.

**CI/CD updates required**:
- Update `jest.config.js` coverage thresholds: methodology is delete V1 production code and tests in the SAME PR, run full suite (`npm test -- --coverage`), record new baseline, set thresholds at `baseline - 5%` for each metric. Include `jest.config.js` changes in the PR to ensure CI runs full suite (not `--changedSince`). **Timing**: recalibrate thresholds in the FINAL M8 PR (PR 7) using the post-deletion baseline. Intermediate deletion PRs set a coverage floor of 40% (not 0%) to catch accidental regressions while allowing expected fluctuation from mass deletion. Each intermediate PR must verify `npm test -- --coverage` passes with the 40% floor before merge.
...
- **V1 types.ts**: NOT deleted in M8. Contains types used by kept production code (service actions, tournament.ts, etc.). M8 may prune V1-only type exports no longer imported after agent/pipeline deletion (verify via `grep -r` first), but the file persists until all consumers are migrated.
...
- `supabase/migrations/20260315000001_evolution_v2.sql` (~350 LOC) — Drops all V1 evolution tables + RPCs, then creates V2 schema:
  ...
  - **V2.1 Arena** (3 tables): `evolution_arena_topics` (prompts, case-insensitive unique), `evolution_arena_entries` (Elo merged in — no separate elo table; includes `archived_at TIMESTAMPTZ` for soft-delete; `run_id UUID REFERENCES evolution_runs(id) ON DELETE SET NULL` — nullable because manual entries have no associated run; SET NULL so arena entries outlive deleted runs), `evolution_arena_comparisons` (minimal: topic_id FK, `entry_a UUID REFERENCES evolution_arena_entries(id) ON DELETE CASCADE`, `entry_b UUID REFERENCES evolution_arena_entries(id) ON DELETE CASCADE`, winner, confidence, run_id UUID REFERENCES evolution_runs(id) ON DELETE SET NULL — nullable for admin-initiated comparisons with no associated run, `status TEXT NOT NULL DEFAULT 'completed' CHECK (status IN ('pending', 'completed', 'failed'))` — pending state used by M10's two-phase rate limit pattern for single comparisons, created_at — topic_id included to avoid join-through-entries for Match History tab queries)
  ...
  - **RPCs** (4, all SECURITY DEFINER with explicit `REVOKE EXECUTE ON FUNCTION <name> FROM PUBLIC; GRANT EXECUTE ON FUNCTION <name> TO service_role;` plus `SET search_path = public` on each function definition to prevent search_path injection — V1 omitted these on sync_to_arena and update_strategy_aggregates, leaving them callable by anon):
    ...
    - `cancel_experiment` (NEW — atomically sets experiment status to `cancelled` + bulk-updates its pending/claimed/running runs to `failed` status. SECURITY DEFINER, service_role only. Used by M11's cancelExperimentAction)
...
- `.github/workflows/ci.yml` — M9 script-deletion PR should run full test suite (not `--changedSince`) to validate transition. Add `supabase db push --dry-run` step (path-filtered to `supabase/migrations/`) to verify the prod deployment path (`supabase db reset` has no `--dry-run` flag).
...
- **Import path fixup**: After moving to `deferred/`, update relative imports in deferred scripts (e.g., `../lib/arenaUtils` → `./lib/arenaUtils` since `lib/arenaUtils.ts` moves into `deferred/lib/`). Verify no non-deferred code imports `lib/arenaUtils.ts` — if it does, keep a re-export shim or move the shared utility to a common location.
...
**Test strategy**:
- **Schema verification**: Run `supabase db reset` with new migration; then run automated SQL assertions against `information_schema.tables` and `information_schema.columns` to verify all 10 V2 tables created with correct columns (not just exit-code success). Verify all indexes exist via `pg_indexes`. Verify FKs enforce referential integrity. Also test `supabase db push --dry-run` against a linked remote to verify the prod deployment path (push, not just reset). Note: `supabase db reset` has no `--dry-run` flag — use `supabase db push --dry-run` for that purpose.
- **RPC test cases**:
...
- `evolution/src/lib/v2/arena.ts` (~150 LOC) — Core Arena functions:
  ...
  - `syncToArena(runId, promptId, pool, ratings, matchHistory, supabase)` — Full sync via `sync_to_arena` RPC:
    ...
    - **Upsert conflict key**: `ON CONFLICT (id)` — each variant has a unique ID (primary key) generated at creation time, not content-based. Two variants with identical text but different IDs are distinct arena entries. The `topic_id` is included in the INSERT columns but not the conflict target since `id` alone is unique.
...
**Test strategy** (~360 LOC across 3 test files, 20 unit tests + 3 E2E + 9 integration):
...
- 20 unit tests + 3 E2E tests + 9 integration tests passing
...
**Rollback plan**: M10 is deployed as a single atomic PR. If deployment fails: (1) revert the PR (git revert) — this restores M10's rewritten arenaActions.ts to its M6-era state (adminAction wrappers with all 14 actions including 6 dead ones) and restores V1 admin pages; (2) M9 arena tables remain (inert without M10 code calling them — no harm); (3) V1 arenaIntegration.ts and pipeline.ts are already deleted by M8, so the restored V1 arena actions have no pipeline caller — they are orphaned but harmless (admin pages still work since they call actions directly, not via pipeline). Rollback restores a functional but degraded state: V1 admin pages render, V1 actions work for direct admin use, but no pipeline arena integration exists until M10 is re-landed. The critical invariant: M10 must not delete V1 arena code until V2 replacements are tested and working in the same PR.
...
**Eliminated** (11 V1 actions not reimplemented in V2.2 — 3 actively deleted in M11's "Done when" since M11 replaces their consuming pages; remaining 8 become dead code when M11's admin pages replace V1 pages, bulk dead-code sweep in M8): archiveExperiment, unarchiveExperiment, startManualExperiment, regenerateReport, getExperimentName, renameExperiment, getExperimentMetrics (separate), getStrategyMetrics, getRunMetrics (separate), getActionDistribution, deleteExperiment
