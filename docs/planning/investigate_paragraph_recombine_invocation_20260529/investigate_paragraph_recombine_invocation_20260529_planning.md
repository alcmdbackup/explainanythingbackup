...
- [x] **Option E: UI "Iteration" column** — DONE (folded into Phase 2): `hideIterationColumn` prop on `ArenaLeaderboardTable`, passed by `SlotsTab` so the always-0 `generation` column is hidden for paragraph topics.
...
- [x] `evolution/src/lib/pipeline/finalize/persistRunResults.test.ts` — **article-path no-clobber regression** (shared-payload safety): static assertion on migration `20260529000001` — `ON CONFLICT DO UPDATE SET` excludes `parent_variant_ids`/`match_count` (so re-syncing an article variant doesn't clobber finalize-written values) while the INSERT branch DOES include them. (Implemented as a SQL-content guard since the RPC is mocked in unit tests; the runtime conflict behavior is additionally covered by the integration round-trip in CI.) assert that when an article variant already has `parent_variant_ids`/`match_count` set (finalize path) and re-enters via `syncToArena`'s INSERT…ON CONFLICT, those two columns are NOT overwritten (verify they're absent from the `ON CONFLICT DO UPDATE SET` list — assert via the generated payload/SQL or a mock RPC capturing the entry shape).
