...
- [ ] Author a dated seed script under `evolution/scripts/experiments/seedElo<...>Experiment_20260626.ts` cloned from `seedCoherencePassPerformanceExperiment_20260624.ts` (research KF4): generalized `buildConfig` to 9 arms, each the **single-round, repeat-to-budget, off-seed** structure (Decision D) — one iteration `{ agentType=X, sourceMode:'seed', budgetPercent:100 }`, config-hash-distinct only on `agentType`, hold-constant set per KF3, criteria arms reuse generic criteria UUIDs (KF7). **Configure each arm to consume ~full budget** (Decision B): set `maxDispatches` high for capped (paragraph) agents, keep top-up on. Creates the experiment, queues runs interleaved at equal `budgetUsd`+`budget_cap_usd`. Cost tracking enforced downstream (do NOT bypass `upsert/create/addRun`). Add README index row.
