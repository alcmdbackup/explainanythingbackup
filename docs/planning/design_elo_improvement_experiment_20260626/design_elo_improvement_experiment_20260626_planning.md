...
- [x] **INFERENCE — P(best) / top tier (see #4 sub-decision 1):** bootstrap that (1) **resamples runs** within each arm AND (2) **samples each variant's Elo from `Normal(elo, sigma)`** before taking the per-run max, computes each arm's **median** maxLift in the replicate, and tallies which arm is highest → **P(best) per arm**. Sigma sampling is RETAINED (user) so per-variant rating uncertainty propagates into the confidence, not just run-to-run variance.
