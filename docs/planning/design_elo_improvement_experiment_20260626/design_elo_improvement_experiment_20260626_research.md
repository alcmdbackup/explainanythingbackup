...
4. **Primary DV** — RESOLVED (user, 2026-06-26): **ceiling = per-run max over the run's OWN variants' point Elos, lift over seed, at equal budget** (matches "keep the winner"); no hard noise guard. **Inference = bootstrap P(best)/top tier** (sub-decision 1, Option D) that resamples runs AND samples each variant Elo from `Normal(elo,sigma)` (sigma retained per user; mild Jensen tilt accepted, shrinkage pilot-gated). Secondary lens: each arm vs `generate` baseline. Secondaries: mean/`eloAttrDelta`, P(beat seed), Elo-per-$, variant_count. See planning Decision C + #4 notes.
