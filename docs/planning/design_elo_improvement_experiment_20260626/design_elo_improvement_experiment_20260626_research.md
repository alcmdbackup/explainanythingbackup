...
6. **Run count / power**: pilot (3–5/arm) to estimate **cross-run sigma** (SD of per-run max-lift across an arm's runs), then size for ~80% power to detect ~40 Elo via formula + P(best) simulation. Total budget ≈ arms × runs × ~$0.10–0.30 × 1.5 margin (well within $25/day staging cap). [Terminology: cross-run sigma vs variant sigma — see planning Decision C note.]
