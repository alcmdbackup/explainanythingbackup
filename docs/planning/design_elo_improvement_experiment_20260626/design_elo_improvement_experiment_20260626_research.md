...
4. **Primary DV** — RESOLVED (user, 2026-06-26): **ceiling = per-run max Elo lift over the seed, at equal budget** (matches "keep the winner"), with a lower-confidence-bound noise guard on "best". Secondaries: mean/`eloAttrDelta`, P(beat seed), Elo-per-$, variant_count. See planning Decision C.
