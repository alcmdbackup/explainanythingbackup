...
- [ ] **1.6** Phase enum sync across **both** TS sites + standalone startup assertion (per Round 3 review pass-2 follow-up — assertion must run unconditionally, not gated behind `COST_CALIBRATION_ENABLED`):
   ...
   - `evolution/src/lib/core/costCalibrationLoader.ts:24` — extend `CalibrationRow['phase']` literal-type union with the same 4 additions. Without this extension, Phase 1.7's cost estimator calls into the loader fail to typecheck.
   - **NEW module** `evolution/src/lib/core/startupAssertions.ts` (~50 LOC) — standalone deploy-ordering gate, invoked once from `agentRegistry.ts` lazy-init path BEFORE returning agent classes:
     - Function `assertCostCalibrationPhaseEnumsMatch()`. Runs `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname = 'evolution_cost_calibration_phase_check'`.
     - Parses the IN-list; asserts every value in the LOCAL `Phase` literal union AND `CalibrationRow['phase']` literal union is present in the DB CHECK.
     - On mismatch: throw `MissingMigrationError` naming the missing values and the migration file(s) expected to add them. Refuses to return the agent class; service startup fails fast.
     ...
     - Unit test `startupAssertions.test.ts` (~80 LOC): mock `pg_get_constraintdef` returning a CHECK with one phase missing → assertion throws with named missing phase; CHECK matching exactly → returns silently; CHECK has extra phases not in TS → that's fine, doesn't throw (DB-superset-of-TS is allowed during rollout).
...
- [ ] **3.7** Integration test `evolution/src/__tests__/integration/iterative-editing-agent.integration.test.ts` (real DB) — assertions corrected per Round 3 review pass-2 follow-up to match Decisions §14 (editing emits no per-cycle pool comparisons in v1):
...
- [ ] **3.8** Sample-article integration test `evolution/src/__tests__/integration/iterative-editing-sample-articles.integration.test.ts` (real DB, ~250 LOC) — assertions corrected per Round 3 review pass-2 follow-up:
