...
- [ ] **1.1** `evolution/src/lib/schemas.ts:394` — extend `iterationAgentTypeEnum` (currently 3 values post-PR-1017: `['generate', 'reflect_and_generate', 'swiss']`) with `'iterative_editing'` → 4 values. Update existing refines on `iterationConfigSchema` (lines 413–447) to allow iterative_editing iterations (forbid as first iteration — the existing first-iteration refine using `isVariantProducingAgentType()` already blocks editing-first since editing is NOT variant-producing). Add three new fields:
...
- [ ] **1.4** `evolution/src/lib/core/metricCatalog.ts` + `evolution/src/lib/metrics/registry.ts` — 1 during-execution cost def + 2 cost propagation defs (mirror `generation_cost` pattern) + 3 propagation defs for the operational-health metrics. Each operational-health metric includes a threshold annotation (`alertWhen`) used by the run-detail dashboard: `accept_rate alertWhen > 0.95` (rubber-stamping signal), `drift_rate alertWhen > 0.30`, `recovery_success_rate alertWhen < 0.70`. Threshold rendering is best-effort; firing alerts is out of scope (the dashboard simply colors out-of-band values red).
- [ ] **1.5a** New migration `supabase/migrations/<timestamp>_evolution_cost_calibration_reflection_phase.sql` — extend `evolution_cost_calibration.phase` CHECK to accept `'reflection'`. Forward-only. Fixes the silent-reject bug PR #1017 left behind (TS enum updated, CHECK migration missing). **Independent of editing** — can ship and revert without touching editing-phase additions. See Decisions §18.
- [ ] **1.5b** New migration `supabase/migrations/<timestamp+1s>_evolution_cost_calibration_editing_phases.sql` — extend the same CHECK to accept `'iterative_edit_propose'`, `'iterative_edit_review'`, and `'iterative_edit_drift_recovery'`. Forward-only. Independent rollback path from 1.5a. See Decisions §18.
- [ ] **1.6** `evolution/scripts/refreshCostCalibration.ts` — add the three editing phases (`'iterative_edit_propose'`, `'iterative_edit_review'`, `'iterative_edit_drift_recovery'`) to the `Phase` literal type and `asPhase()` mapping. **Add startup CHECK-constraint assertion** (~30 LOC, mitigates Round 3 review S5 + T2 — recurrence of PR #1017's silent-reject bug):
   - On loader init, run `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname = 'evolution_cost_calibration_phase_check'` (or equivalent).
   - Parse the IN-list; assert every value in the local `Phase` literal type union is present.
   - On mismatch (a phase string in TS that's NOT in the DB CHECK): throw a loud startup error naming the missing values and the migration file expected to add them. Block agent registry initialization.
   - This is a **deploy-ordering gate**: code that writes new phase strings cannot start before the migration runs, eliminating the silent-reject failure mode.
   - Unit test: integration test loading a fixture migration with one phase missing → assertion throws with the expected error message naming the missing phase.
...
- [ ] **1.10** Build `evolution/src/lib/pipeline/loop/editingDispatch.ts` (~60 LOC) — the **shared dispatch helper for editing iterations**, mirroring PR #1017's `reflectionDispatch.ts` pattern (Round 3 review A4 fix). Single source of truth for "given an iteration config + a candidate pool, who is eligible for editing?":
   - `resolveEditingDispatch(args: { iterCfg: IterationConfig; pool: Variant[]; arenaVariantIds: Set<string>; iterationStartRatings: Map<variantId, Rating> }): { eligibleParents: Variant[]; cutoffMode: 'topN' | 'topPercent'; cutoffValue: number; effectiveCap: 'eligibility' | 'pool_size' | 'unbounded'; droppedReason?: string }`.
   - Pure function, no side effects, no DB access.
   - Algorithm: filter out arena entries; sort by Elo descending using `iterationStartRatings`; apply `iterCfg.editingEligibilityCutoff` (`topN` slice or `topPercent ⇒ ceil(len * value/100)`); return slice + `effectiveCap`.
   - Used by Phase 3.3 (runtime dispatch in `runIterationLoop`) AND Phase 3.4 (planner in `projectDispatchPlan`). Both phases call this single function — no inline arithmetic in either.
   - Unit test `editingDispatch.test.ts` (~120 LOC, ~10 cases): topN with pool larger than cutoff, topN with pool smaller than cutoff, topPercent rounding, all-arena pool (eligibleParents empty), Elo tie-breaking deterministic, missing rating treated as -inf, cutoff ≤ 0 rejected (validator-side concern but defensive guard test).
...
- [ ] **2.A.1** Create `evolution/src/lib/core/agents/editing/IterativeEditingAgent.ts` (~280 LOC). Extend `Agent<IterativeEditInput, IterativeEditOutput, IterativeEditingExecutionDetail>`. Set `usesLLM = true`, `name = 'iterative_editing'`.
   ...
   - **Base-class contract conformance** (per Round 3 review A5 — every variant-producing agent inherits a set of B0xx contracts; this agent must enumerate which apply, are inverted, or are no-ops):

     | Contract | Source | Behavior in `IterativeEditingAgent` |
     |---|---|---|
     | `enforceVariantFormat()` (B047) | `Agent` base | **Per-cycle**, applied to each cycle's `newText` BEFORE writing intermediate `childText` to `execution_detail`. Format failure aborts the cycle with `stopReason: 'format_invalid'` (cycle is dropped; remaining cycles do NOT run). The final variant emitted from `execute()` is also format-validated by the base class on output as usual. |
     | `surfaced` flag flow (B048) | `AgentOutput.surfaced` | Set to `true` when at least one cycle produced an accepted edit AND format-valid AND `appliedCount > 0`. Set to `false` when all cycles ended in `all_edits_rejected` / `no_edits_proposed` / `parse_failed` / drift-abort / size-explosion. The flag flows through `updateInvocation` to `evolution_variants.variant_surfaced` exactly as for generate. |
     | `tactic` auto-extraction (B051) | `Agent.ts:52` | **Inverted (no-op).** `IterativeEditInput` does NOT carry a `tactic` field — the editing agent does not apply a tactic. The `tactic` column on `evolution_agent_invocations` is `null` for editing rows, and tactic-cost rollups must filter editing rows out (or treat them as a separate bucket). Phase 4.7 (`CostEstimatesTab`) handles this via the `name.includes('edit')` branch. |
     | FK threading (B054) | `createEvolutionLLMClient` | **Same as generate.** Wrapper-instantiated `EvolutionLLMClient` carries `invocationId`; all 2–3 LLM helper calls per cycle reuse this client (per Decisions §13 invariant I1) and write `evolution_llm_call_tracking` rows linked to the wrapper's invocation row. |
     | Single variant per `AgentOutput` (B0xx implicit) | `AgentOutput<TOutput>` shape | **Preserved** per Decisions §14. Only the final cycle's text materializes as a `Variant` in the pool. Intermediate cycles' `childText` is captured in `execution_detail.cycles[i].childText` only — no `evolution_variants` rows for intermediates, no `evolution_arena_comparisons` rows, no Elo. The final variant's `parent_variant_id` is the original `input.parent.variantId`, NOT cycle-N-1's intermediate. |
     | Partial `execution_detail` write on throw (B0xx) | `trackInvocations.ts` partial-update path | **Required** per Decisions §13 invariant I3. Wrap the cycle loop in try/catch; on any helper throw, write `execution_detail.cycles[]` accumulated so far + `stopReason: 'helper_threw'` + `errorMessage` BEFORE re-throwing. Mirrors `reflectAndGenerateFromPreviousArticle.ts:444-470`. |

     If a future contract is added to `Agent` base (B055+), this table must be extended in the same edit. Phase 2.A.5 invariant test asserts the table includes every public contract surface enumerated in `Agent.ts`.
...
- [ ] **2.A.5** Invariant test `IterativeEditingAgent.invariants.test.ts` (~150 LOC, mirrors `reflectAndGenerateFromPreviousArticle.invariants.test.ts`):
   ...
   - Assert no `new ReflectAndGenerateFromPreviousArticleAgent\\(` / `new GenerateFromPreviousArticleAgent\\(` / `\\.run\\(` calls inside `IterativeEditingAgent.execute()` body (regex on file text — guards Decisions §13 invariant I1).
   - Assert all 3 helper call sites (propose / approve / recoverDrift) are immediately preceded by a `costBefore*` capture (regex pattern check — guards I2).
   - Assert the cycle loop body is wrapped in try/catch with a partial-detail write before re-throw (guards I3).
   - Assert the base-class contract conformance table from Phase 2.A.1 covers every public contract surface enumerated in `Agent.ts` (parse `Agent.ts` for B0xx contract markers; cross-reference).
...
- [ ] **3.2** Widen `recordSnapshot()` `iterationType` union at `runIterationLoop.ts:91` from the current 3 values (`'generate' | 'reflect_and_generate' | 'swiss'`, post-PR-1017) to 4 by adding `'iterative_editing'`. Update the 4 existing call sites (lines 313, 686, 699, 786, 809 — confirm exact line numbers at edit time; PR #1017 added the reflect_and_generate branch and shifted them) and add new call sites at iteration `start`/`end` of the new editing branch from Phase 3.3.
- [ ] **3.3** Add new `else if (iterType === 'iterative_editing')` branch in `runIterationLoop.ts` (~160 LOC). Mirrors the existing generate branch's parallel + top-up pattern; the only difference is what each invocation receives as input.
   ...
   - **Top-up loop** (gated by `EVOLUTION_TOPUP_ENABLED`): after parallel batch resolves, measure `actualAvgCostPerAgent` from the parallel agents' `scope.getOwnSpent()`. While remaining iter budget covers another invocation AND there are still un-edited parents in the pool (next-best Elo not yet assigned), dispatch one more invocation with `perInvocationBudgetUsd = remainingBudget` (top-up gets the full remainder since it's the last invocation). Top-up parents pull from the next Elo rank not yet edited this iteration via the same `resolveEditingDispatch` ordering. Cap at `DISPATCH_SAFETY_CAP = 100` like generate.
...
- [ ] **3.3.1** Widen `MergeRatingsInput.iterationType` enum (per Decisions §7) at `evolution/src/lib/core/agents/mergeRatings.ts` from `'generate' | 'swiss'` to `'generate' | 'iterative_editing' | 'swiss'`. The merge math inside `MergeRatingsAgent` is unchanged — editing matches are merged identically to generate matches. Update `MergeRatingsAgent.execution_detail.iterationType` field schema to match. Update unit tests + fixtures that construct `MergeRatingsInput` literals to include the new variant where applicable.
- [ ] **3.4** `evolution/src/lib/pipeline/loop/projectDispatchPlan.ts` — add `iterative_editing` case using `estimateIterativeEditingCost(seedChars, editingModel, approverModel, driftRecoveryModel, maxCycles)`. **Eligibility-cutoff arithmetic uses the shared helper** (per Round 3 review A4): call `resolveEditingDispatch({ iterCfg, pool: projectedPoolAtIterationStart, arenaVariantIds, iterationStartRatings })` from Phase 1.10 — same function the runtime uses, applied to the planner's projected pool. Cost-per-invocation is the editing-agent cost (proposer + approver + maybe drift recovery, × maxCycles); dispatch count = floor(iterBudget / costPerInvocation), capped at `DISPATCH_SAFETY_CAP`, **at the helper's `eligibleParents.length`** (NOT inline arithmetic), and at the iteration-start pool size (excluding arena entries). Returns the same `IterationPlanEntry` shape as generate so the wizard preview renders consistently. Set `effectiveCap: 'eligibility'` on the entry when the cutoff is the binding ceiling, so the wizard preview can flag it ("cutoff binding — N invocations of budget unused").
...
- [ ] **3.7** Integration test `evolution/src/__tests__/integration/iterative-editing-agent.integration.test.ts` (real DB):
   ...
   - Assert: `evolution_agent_invocations` row written with `agent_name='iterative_editing'`; `execution_detail` validates against schema; `evolution_arena_comparisons` row written for each accepted edit; `evolution_variants` row created for accepted variant; `iterative_edit_cost` metric > 0.
...
- [ ] **5.2** `evolution/src/components/evolution/DispatchPlanView.tsx:117–119` — add badge color for `'iterative_editing'`. Show `cycles × top-K` in the dispatch entry tooltip when set.
...
- [ ] **6.1** E2E spec `src/__tests__/e2e/specs/09-admin/admin-evolution-iterative-editing.spec.ts` — uses the **established cross-process LLM mock pattern** (`v2MockLlm`), NOT `nock` (per Round 3 review T1 — `nock` only intercepts in-process `fetch` and cannot reach the Next.js server process started by `ensure-server.sh`):
   - Seed strategy via service-role with 1×generate + 1×editing iteration; budget $0.05. Configure `editingModel` and `approverModel` to differing values to avoid the rubber-stamping warning (and a second test variant where they match, to assert the warning surfaces).
   - **LLM mock setup**: server reads `EVOLUTION_USE_V2_MOCK_LLM=true` env var (set by `ensure-server.sh` in test mode) and routes all LLM provider calls through `v2MockLlm` instead of the real provider. Spec authors fixture responses keyed by `AgentName + cycleNumber` (e.g., `iterative_edit_propose:1`, `iterative_edit_review:1`, `iterative_edit_propose:2`, …). Fixture file lives at `evolution/src/testing/fixtures/editing-e2e-responses.ts`.
   - Trigger via `/api/evolution/run`.
   - Poll DB until run status = 'completed'.
   - Navigate to run detail → Variants tab → assert exactly **one** new variant appears per editing invocation (per Decisions §14 — single-final-variant), with `parent_variant_id` pointing at the original generated parent (NOT cycle-N-1's intermediate).
   - Navigate to invocation detail for the editing invocation → assert `cycles[]` table renders with `proposeCostUsd` / `approveCostUsd` columns populated, the `<AnnotatedProposals>` field renders accepted edits in green and rejected in red, and `<TextDiff>` visible with `parentText` (cycle 1's input) and `childText` (cycle 1's output).
   - Assert **rubber-stamping warning** appears in Step 1 of strategy wizard when `approverModel === editingModel` (resolved values), and disappears when the user picks differing models (per Decisions §16).
- [ ] **6.1.1** Backward-compatibility test for the schema discriminator rename (per Round 3 review T4): `evolution/src/__tests__/integration/iterative-editing-schema-bc.integration.test.ts` (~80 LOC):
   - Insert a synthetic `evolution_agent_invocations` row with `execution_detail.detailType === 'iterativeEditing'` (the V1 orphaned shape, mirroring the orphaned `iterativeEditingDetailFixture` pre-rewrite).
   - Confirm Phase 1.8's schema rewrite either: (a) parses it via a backward-compat union member, OR (b) the schema rejects it cleanly with a non-crashing validation error that surfaces in `InvocationListEntity` rather than blowing up. Pick (b) since orphaned scaffolding was never written to in production — but assert the failure mode is graceful (no 500-page on the admin UI, no migration loader crash).
   - Add a parallel test for `MergeRatingsAgent.execution_detail.iterationType` rows persisted with the old `'generate'` value before our rename — assert they still parse via the widened enum (per Decisions §7 widening keeps `'generate'` as a valid variant).
...
| Orphaned schema drift (S1, T1) | Phase 1.8 fixture-validation test runs at bootstrap. Phase 6.1.1 BC test confirms graceful failure for V1-shape rows. |
...
| Critique amplification (B1) | Add critique-quality validation in Phase 2.6; log rejected cycles for staging analysis. Operational metric `iterative_edit_accept_rate` alerts when persistently > 0.95. |
...
Three independent rollback paths, none requiring DB downgrade:

- **Soft rollback (operational)**: set `EDITING_AGENTS_ENABLED='false'`. No code or migration change. Editing iterations short-circuit; runs that already started keep going to completion (mid-run flag flips do not abort). Use for cost spikes, drift-rate spikes, accept-rate runaway.
- **Drift recovery rollback**: set `EVOLUTION_DRIFT_RECOVERY_ENABLED='false'`. Drift recovery LLM call is skipped; minor drift is treated as major (cycle aborts). Use when recovery LLM goes haywire.
- **Code rollback (hard)**: revert the code commits, leave both migrations in place. The CHECK constraint extensions are non-destructive; existing reflection / generate / ranking phase rows continue to work. The migrations stay forward-only by design — there is no down-migration. If a future need genuinely requires removing the editing phases from CHECK, that's a separate destructive migration and would need its own RFC.

### What we explicitly do NOT support

- **Migration revert**: the CHECK extensions are forward-only. Reverting `1.5a` would re-introduce PR #1017's silent-reject bug for reflection users — explicitly disallowed. Reverting `1.5b` would silently drop editing cost-calibration rows — also disallowed. Hard rollback uses the feature flag, not migration revert.
- **Mid-run flag flip aborting in-flight runs**: `EDITING_AGENTS_ENABLED` is read at iteration-loop entry, not per-cycle. A run that has started its editing iteration when the flag flips OFF will complete that iteration. This is intentional — partial-iteration aborts produce broken audit trails and cost-attribution gaps.
