...
6. **Parent selection:** **One parent per `IterativeEditingAgent` invocation** (always K=1 internally). Multiple invocations dispatch in parallel via the existing generate-iteration dispatch framework (`projectDispatchPlan` budget-governed parallel batch + top-up loop). Each invocation in the iteration is assigned a distinct top-N variant by Elo rank from the iteration-start pool. If parallel-dispatch count exceeds available unique parents in the pool, the dispatch caps at the pool size. No `editingTopK` config field — the dispatch count comes from the same place generate iterations get it.
...
12. **Editing-specific config knobs (strategy-level + per-iteration):**
    ...
    - **Parallel dispatch count is NOT configurable per editing iteration.** It's derived by `projectDispatchPlan` from the iteration's budget — same path generate iterations use. Each parallel invocation edits one distinct top-N parent. (No `editingTopK` field.)
...
- [ ] **1.1** `evolution/src/lib/schemas.ts:388` — extend `iterationAgentTypeEnum` with `'iterativeEditingAgent'`. Update 4 refines on `iterationConfigSchema` (lines 413–425) to allow iterativeEditingAgent iterations (forbid as first iteration). Add one new optional field on `iterationConfigSchema` and one on `strategyConfigBaseSchema`:
   - On `iterationConfigSchema`: `editingMaxCycles: z.number().int().min(1).max(5).optional()` — per-iteration override for how many cycles each editing-agent invocation runs (default falls through to `AGENT_DEFAULT_MAX_CYCLES = 3`). Refine: only allowed when `agentType === 'iterativeEditingAgent'`. **No `editingTopK` field** — parallel dispatch count comes from `projectDispatchPlan` via the same budget-governed mechanism generate iterations use.
...
- [ ] **3.3** Add new `else if (iterType === 'iterativeEditingAgent')` branch in `runIterationLoop.ts` (~150 LOC). Mirrors the existing generate branch's parallel + top-up pattern; the only difference is what each invocation receives as input.
   - **Compute parent assignment**: take `iterationStartPool` (the snapshot at iteration start), filter out arena-sourced variants, sort descending by Elo, take the top N where N = `min(filteredPool.length, parallelBatchSize)` and `parallelBatchSize` comes from `projectDispatchPlan` (budget-governed). Each parent in this slice will be the input to one parallel agent invocation.
   ...
   - `recordSnapshot(iterIdx, 'iterativeEditingAgent', 'start'/'end', ...)`.
- [ ] **3.4** `evolution/src/lib/pipeline/loop/projectDispatchPlan.ts` — add `iterativeEditingAgent` case using `estimateIterativeEditingCost(seedChars, editingModel, driftRecoveryModel, maxCycles)`. Cost-per-invocation is the editing-agent cost (proposer + approver + maybe drift recovery, × maxCycles); dispatch count = floor(iterBudget / costPerInvocation), capped at `DISPATCH_SAFETY_CAP` and at the iteration-start pool size (excluding arena entries). Returns the same `IterationPlanEntry` shape as generate so the wizard preview renders consistently.
...
- [ ] **5.3** `evolution/src/services/strategyPreviewActions.ts:159–185` — extend `dispatchPreviewInputSchema` to accept `'iterativeEditingAgent'` and optional per-iteration `editingMaxCycles`. Strategy-level `editingModel` field also added to the preview input.
...
- [ ] **5.5** Surface `editingMaxCycles` in the Step 2 iteration row when `agentType === 'iterativeEditingAgent'`. Single inline input (no collapsible "Edit settings" subsection — only one field):
   ...
   - Parallel dispatch count is intentionally NOT surfaced as a strategy author input — it's derived from the iteration's budget by `projectDispatchPlan`. The dispatch preview underneath shows the resulting count alongside per-iteration cost.
