...
- [ ] **1.1** `evolution/src/lib/schemas.ts:388` — extend `iterationAgentTypeEnum` with `'iterativeEditingAgent'`. Update 4 refines on `iterationConfigSchema` (lines 413–425) to allow iterativeEditingAgent iterations (forbid as first iteration). Add three new fields:
...
- [ ] **3.3** Add new `else if (iterType === 'iterativeEditingAgent')` branch in `runIterationLoop.ts` (~160 LOC). Mirrors the existing generate branch's parallel + top-up pattern; the only difference is what each invocation receives as input.
...
- [ ] **5.3** `evolution/src/services/strategyPreviewActions.ts:159–185` — extend `dispatchPreviewInputSchema` to accept `'iterativeEditingAgent'`, optional per-iteration `editingMaxCycles`, and `editingEligibilityCutoff` (defaults to `{ mode: 'topN', value: 10 }` if absent). Strategy-level `editingModel` field also added to the preview input.
...
- [ ] **5.5** Surface `editingMaxCycles` and `editingEligibilityCutoff` in the Step 2 iteration row when `agentType === 'iterativeEditingAgent'`. Two inline inputs:
