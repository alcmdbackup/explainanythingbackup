...
| File | Purpose |
|------|---------|
| `src/lib/evolution/agents/sectionDecompositionAgent.ts` | Orchestrates parse→parallel-edit→stitch flow |
| `src/lib/evolution/section/sectionParser.ts` | `parseArticleIntoSections()` — regex split at H2 with code block stripping |
| `src/lib/evolution/section/sectionStitcher.ts` | `stitchSections()`, `stitchWithReplacements()` — reassembly with selective replacement |
| `src/lib/evolution/section/sectionFormatValidator.ts` | Relaxed format validator (no H1, H2 heading required, no bullets/lists/tables) |
| `src/lib/evolution/section/sectionEditRunner.ts` | Per-section critique→edit→judge loop using `compareWithDiff()` |
| `src/lib/evolution/section/types.ts` | `ArticleSection`, `ParsedArticle`, `SectionVariation`, `SectionEvolutionState` |
