...
| File | Purpose |
|------|---------|
| `src/lib/evolution/flowRubric.ts` | Flow dimensions, prompt builders, parsers, score normalization, cross-scale targeting |
| `src/lib/evolution/core/pipeline.ts` | `runFlowCritiques()` standalone function and pipeline integration |
| `src/lib/evolution/agents/pairwiseRanker.ts` | Flow comparison mode in `comparePairFlow()` |
| `src/lib/evolution/core/featureFlags.ts` | `flowCritiqueEnabled` flag definition |
