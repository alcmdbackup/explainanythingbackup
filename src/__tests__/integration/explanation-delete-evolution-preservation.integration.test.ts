...
describe('Explanation delete preserves evolution data', () => {
  ...
  it('deletes the explanation but keeps the evolution_run with explanation_id NULL', async () => {
    ...
    // Arrange: create a test topic (explanations.primary_topic_id is NOT NULL).
    const { data: topic, error: topicErr } = await supabase
      .from('topics')
      .insert({
        topic_title: `[TEST] preserve-evolution-topic-${testId}`,
        topic_description: 'test topic for preserve-evolution test',
      })
      .select()
      .single();
    expect(topicErr).toBeNull();
    const topicId = (topic as { id: number }).id;

    // Create a test explanation referencing the topic.
    ...
    // Verify the explanation is truly gone (defensive: silent zero-row deletes
    // would otherwise mask FK behaviour).
