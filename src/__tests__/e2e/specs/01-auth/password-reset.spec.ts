...
test.describe('Password Reset', { tag: '@critical' }, () => {
  ...
  test('Reset flow: recovery link → /reset-password → updateUser → /', async ({ page }) => {
    ...
    // The action_link is the raw GoTrue verify URL. Hitting it directly
    // routes through Supabase, which then redirects to our redirect_to (built
    // from the recovery email template — which we own). The end state must
    // be /reset-password.
    const resetPage = new ResetPasswordPage(page);
    await resetPage.gotoUrl(actionLink!);
