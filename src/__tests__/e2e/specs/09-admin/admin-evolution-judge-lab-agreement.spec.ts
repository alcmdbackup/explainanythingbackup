...
adminTest.describe('Judge Lab · Agreement', { tag: '@evolution' }, () => {
  ...
  adminTest('seeded agreement run renders in the leaderboard, detail, and matches sub-route', async ({ adminPage }) => {
    ...
    // Tile labels appear in the tile + methodology + glossary, so getByText would multi-match
    // in strict mode. Each tile carries a stable data-testid that targets it uniquely.
    await expect(adminPage.getByTestId('tile-per-pair')).toBeVisible();
    await expect(adminPage.getByTestId('tile-per-repeat')).toBeVisible();
    await expect(adminPage.getByTestId('tile-both-decisive')).toBeVisible();
    await expect(adminPage.getByTestId('tile-abstain')).toBeVisible();
    await expect(adminPage.getByTestId('tile-holistic-pos-bias')).toBeVisible();
    await expect(adminPage.getByTestId('tile-rubric-pos-bias')).toBeVisible();
    ...
    // Wait for rows to actually render — the table can be visible while the action's
    // initial fetch is still in-flight, so `rows.count()` is racy without an auto-waiting
    // assertion. Seeded fixture has exactly 2 rows (one per repeat).
    await expect(rows).toHaveCount(2, { timeout: 15000 });

    // Disagree filter narrows to the both-decisive opposite-winner call (repeat 1) → 1 row.
    // The checkbox is controlled by URL state via router.push, so its `checked` attribute lags
    // the click. Use plain click() + URL/count assertions (auto-waiting) rather than check()
    // which verifies the attribute synchronously.
    await adminPage.getByTestId('agreement-disagree-only').click();
