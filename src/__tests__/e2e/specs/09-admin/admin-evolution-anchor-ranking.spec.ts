...
adminTest.describe('Arena Leaderboard Anchor Badge', { tag: '@evolution' }, () => {
  ...
  adminTest.beforeAll(async () => {
    ...
    const { data: prompt, error: pErr } = await supabase
      ...
      .select('id')
      .single();
    if (pErr || !prompt) throw new Error(`Seed prompt failed: ${pErr?.message ?? 'null data'}`);
    ...
    const { data: strategy, error: sErr } = await supabase
      ...
      .select('id')
      .single();
    if (sErr || !strategy) throw new Error(`Seed strategy failed: ${sErr?.message ?? 'null data'}`);
    ...
    const { data: run, error: rErr } = await supabase
      ...
      .select('id')
      .single();
    if (rErr || !run) throw new Error(`Seed run failed: ${rErr?.message ?? 'null data'}`);
