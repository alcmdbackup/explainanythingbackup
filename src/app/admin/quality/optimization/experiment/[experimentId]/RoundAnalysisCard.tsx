...
export function RoundAnalysisCard({ round }: RoundAnalysisCardProps) {
  const analysis = round.analysisResults as {
  ...
  const statusColor = ROUND_STATUS_COLORS[round.status] ?? 'var(--text-muted)';
    ...
    <div className="border border-[var(--border-default)] rounded-page overflow-hidden bg-[var(--surface-secondary)]">
      <div className="flex items-center justify-between p-3 border-b border-[var(--border-default)]">
        ...
        <div className="flex items-center gap-3 text-xs font-mono text-[var(--text-secondary)]">
          ...
          {round.runCounts.failed > 0 && (
            <span className="text-[var(--status-error)]">{round.runCounts.failed} failed</span>
          )}
          {round.completedAt && (
      ...
      {analysis ? (
        ...
        <div className="p-3 text-xs font-body text-[var(--text-muted)]">
          {round.status === 'completed' ? 'No analysis results available.' : 'Analysis pending.'}
        </div>
      )}
    </div>
  );
