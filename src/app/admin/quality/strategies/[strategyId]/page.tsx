...
import { StrategyConfigDisplay } from '../../optimization/_components/StrategyConfigDisplay';
...
export default async function StrategyDetailPage({ params }: Props) {
  ...
  const avgElo = runs.filter(r => r.finalElo != null).reduce((s, r) => s + (r.finalElo ?? 0), 0)
    / (runs.filter(r => r.finalElo != null).length || 1);
  ...
  return (
    <div className="space-y-6 pb-12">
      <EvolutionBreadcrumb
        items={[
          { label: 'Strategies', href: '/admin/quality/strategies' },
      ...
      <div className="bg-[var(--surface-secondary)] paper-texture rounded-book p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <StatCell label="Avg Elo" value={runs.some(r => r.finalElo != null) ? avgElo.toFixed(0) : '—'} />
