...
export default function AdminCostsPage(): React.ReactElement {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Under-count warning — shown ONLY when the canonical merge is OFF (totals would otherwise
          under-count evolution spend). When merged, the totals already include the invocation-based
          evolution spend, so no banner is needed — the correct number speaks for itself. */}
      {!summary?.evolutionMerged
        && reconciliation
        && reconciliation.invocationCost > reconciliation.trackingCost * 1.5 && (
        <div data-testid="admin-costs-audit-gap" className="p-3 bg-amber-500/10 border border-amber-500/50 rounded-md text-sm">
          <span className="text-amber-500 font-medium">Evolution spend under-counted here.</span>{' '}
          <span className="text-[var(--text-secondary)]">
            Totals show {formatCost(reconciliation.trackingCost)}, but evolution actually spent{' '}
            {formatCost(reconciliation.invocationCost)} this range.
          </span>
        </div>
      )}
