...
export function ResetPasswordForm() {
  ...
  // PASSWORD_RECOVERY is emitted by supabase-js only on the client that *itself*
  // calls verifyOtp — JWT carries amr.method='otp' with no recovery marker, so a
  // cookie-hydrated client sees SIGNED_IN instead. So /auth/confirm forwards us
  // here with token_hash + type=recovery in the URL and we do the verify here.
  // Strip the params afterwards so they don't survive a reload or get logged.
  // On verifyOtp failure the form stays disabled and the invalid-link message
  // already covers the user-facing path.
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const { error } = await supabase_browser.auth.verifyOtp({
        type: 'recovery',
  ...
  const onSubmit = async (data: ResetPasswordInput) => {
    ...
    setFormError(null);
    try {
      // Capture the recovery session's email before updateUser, since the
      // recovery session is consumed once the password changes.
      ...
      // Re-sign-in with the new password. signInWithPassword mints a fresh
      // standard session via @supabase/ssr's cookie write path so the
      // subsequent navigation lands the user authenticated at /.
      if (email) {
        const { error: signInErr } = await supabase_browser.auth.signInWithPassword({
          email,
          password: data.password,
        });
        if (signInErr) {
      ...
      // Hard navigation. router.push() triggers an RSC fetch that was
      // observed not to pick up cookies supabase-js just wrote, so middleware
      // saw no user and bounced to /login. window.location.assign forces a
      // full document GET with current cookies, which middleware reads
      // correctly. Confirmed empirically against prod build + staging
      // Supabase.
