...
const _getExplanationByIdAction = async function(params: { id: number }) {
    // E2E test mode: return mock data for mock IDs (>= 9_000_000).
    // INVARIANT: the 9_000_000 sentinel must stay ABOVE the max real explanation
    // id. test-mode.ts and api-mocks.ts generate mock ids in the 9_000_000+ range.
    // The threshold was previously 90000, which started colliding with real
    // seeded explanations once the dev DB's max id grew past 90000 (~120k now) —
    // real seeded rows were served "Test Explanation Title" instead of their
    // content, breaking content-viewing E2E tests only under E2E_TEST_MODE (CI).
    if (process.env.E2E_TEST_MODE === 'true' && params.id >= 9_000_000) {
        // Match Playwright mock data from api-mocks.ts for specific IDs
        // This ensures loadExplanation returns the same title as streaming mocks
        const mockTitles: Record<number, string> = {
            9_000_001: 'Understanding Quantum Entanglement',  // defaultMockExplanation
            9_000_002: 'Brief Explanation',                   // shortMockExplanation
        };
        const mockContent: Record<number, string> = {
            9_000_001: `# Understanding Quantum Entanglement
...
This fascinating phenomenon challenges our classical understanding of physics and opens doors to revolutionary technologies.`,
            9_000_002: 'This is a short explanation for testing purposes.',
        };
        return {
            id: params.id,
            timestamp: new Date().toISOString(),
            explanation_title: mockTitles[params.id] || 'Test Explanation Title',
            content: mockContent[params.id] || '<p>This is mock explanation content for E2E testing.</p>',
            primary_topic_id: 1,
...
const _resolveLinksForDisplayAction = async function(params: {
    ...
    if (process.env.E2E_TEST_MODE === 'true' && params.explanationId >= 9_000_000) {
...
const _getUserQueryByIdAction = async function(params: { id: number }) {
    // E2E test mode: return mock data for mock IDs (>= 9_000_000)
    // These IDs are generated by test-mode.ts (explanationId + 1000) and don't exist in DB
    if (process.env.E2E_TEST_MODE === 'true' && params.id >= 9_000_000) {
        return {
            id: params.id,
...
const _getAISuggestionSessionsAction = withLogging(
    async function getAISuggestionSessionsAction(
        ...
        try {
            ...
            const uniqueSessions = data?.reduce((acc: typeof data, session) => {
