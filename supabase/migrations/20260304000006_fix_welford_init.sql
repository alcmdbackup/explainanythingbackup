-- Fix Welford mean initialization bug: first-run avg_final_elo should be p_final_elo, not 0.
-- When avg_final_elo is NULL (first run for a strategy), COALESCE(v_old.avg_final_elo, 0)
-- incorrectly starts the running mean at 0 instead of the actual first Elo value.
-- Fix: COALESCE(v_old.avg_final_elo, 0) → COALESCE(v_old.avg_final_elo, p_final_elo).
...
BEGIN
  ...
  -- Welford's online mean update (FIX: use p_final_elo as initial mean, not 0)
