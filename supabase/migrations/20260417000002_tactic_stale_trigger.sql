-- Extend mark_elo_metrics_stale() trigger to cascade staleness to tactic metrics.
-- Preserves the expanded metric list from 20260326000003 + 20260328000002 and adds tactic cascade.
...
BEGIN
  ...
  THEN
    -- Mark ALL run-level finalization metrics stale (preserves 20260326000003 expansion)
    ...
    -- Mark invocation-level elo metrics stale (preserves 20260328000002 expansion)
    UPDATE evolution_metrics SET stale = true, updated_at = now()
    WHERE entity_type = 'invocation' AND entity_id IN (
      SELECT id FROM evolution_agent_invocations WHERE run_id = NEW.run_id
    ) AND metric_name IN ('best_variant_elo', 'avg_variant_elo');

    -- NEW: Cascade to tactic metrics (look up tactic via agent_name → evolution_tactics.name)
    UPDATE evolution_metrics SET stale = true, updated_at = now()
