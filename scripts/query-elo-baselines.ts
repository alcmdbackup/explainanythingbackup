 ...
 * Runs SQL queries against article_bank and llmCallTracking tables.
 */
...
async function queryMethodEffectiveness() {
  ...
  // Query article_bank_entries joined with article_bank_elo
  ...
  if (error) {
    console.error('Error querying article_bank:', error.message);
    return;
  }

  if (!data || data.length === 0) {
    console.log('No article bank entries found. Run scripts/run-prompt-bank.ts first.');
    return;
  }
  ...
  for (const entry of data) {
    ...
    const eloData = Array.isArray(entry.article_bank_elo)
      ? entry.article_bank_elo[0]
      : entry.article_bank_elo;
...
async function main() {
  ...
  console.log('Querying existing data from article_bank and llmCallTracking...');
  ...
  if (methodResults && methodResults.length > 0) {
    console.log(`Found ${methodResults.length} method/model combinations in article_bank`);
    ...
    console.log('No article bank data available for baseline.');
  }
