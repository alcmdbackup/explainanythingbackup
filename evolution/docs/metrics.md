...
| `paragraph_recombine_cost` | cost | during_execution | Umbrella spend on `ParagraphRecombineAgent` — both `'paragraph_rewrite'`-labeled rewrite calls AND the `'ranking'`-labeled per-slot judge calls (routed via per-slot `AgentCostScope` intercept, D11/D18). Per-slot split lives in `execution_detail.slots[*].spentUsd`. Propagates to strategy/experiment as `total_paragraph_recombine_cost` / `avg_paragraph_recombine_cost_per_run`. `listView: true`. (rank_individual_paragraphs_evolution_20260525) |
