...
| `paragraph_recombine_cost` | cost | during_execution | Umbrella spend on `ParagraphRecombineAgent` — both `'paragraph_rewrite'`-labeled rewrite calls AND `'paragraph_rank'`-labeled per-slot ranking calls (the agent relabels `rankNewVariant`'s `'ranking'` calls to `'paragraph_rank'` so they don't pollute `ranking_cost`). Written once per invocation as the SUM of both phase-cost accumulators (MAX-safe — both run-cumulative as of Phase 12 of `analyze_effectiveness_paragraph_recombine_20260530`; pre-Phase-12 the iter tracker returned per-iter, silently shadowing smaller per-iter contributions). Per-slot split lives in `execution_detail.slots[*].spentUsd`. Propagates to strategy/experiment as `total_paragraph_recombine_cost` / `avg_paragraph_recombine_cost_per_run`. `listView: true`. (rank_individual_paragraphs_evolution_20260525) |
| `paragraph_rewrite_estimation_error_pct` | cost | at_finalization | (G7, investigate_paragraph_rewrite_cost_undershoot_evolution_20260529) Mean per-invocation paragraph_rewrite-phase estimation error % across paragraph_recombine invocations. Sourced from `execution_detail.paragraph_rewrite.{estimatedCost,cost}` populated by the agent. Propagates as `avg_paragraph_rewrite_estimation_error_pct`. |
| `paragraph_rank_estimation_error_pct` | cost | at_finalization | (G7) Mean per-invocation paragraph_rank-phase estimation error % across paragraph_recombine invocations. Sourced from `execution_detail.paragraph_rank.{estimatedCost,cost}`. Propagates as `avg_paragraph_rank_estimation_error_pct`. |
| `paragraph_slot_match_persist_failures` | count | during_execution | Number of slots whose `persistSlotMatches()` INSERT failed in this run. Best-effort contract — caught + logged + counted so the run still completes when one slot's persistence fails (others continue). |
| `coordinator_retry_rate` | count | during_execution | (debug_performance_paragraph_recombine_20260612) Fraction of paragraph_recombine invocations whose coordinator (Phase A) first-call parse failed but the single retry succeeded. Expected ~0% at steady state; ramp-up may show non-zero. Propagates as `avg_coordinator_retry_rate`. |
| `coordinator_failure_rate` | count | during_execution | (debug_performance_paragraph_recombine_20260612) Fraction of paragraph_recombine invocations whose coordinator threw (retry also failed). Target <2%; sustained higher values warrant investigation. Propagates as `avg_coordinator_failure_rate`. |
| `excessive_parent_fallback_abort_rate` | count | during_execution | (debug_performance_paragraph_recombine_20260612) Fraction of sequential-mode invocations that aborted Phase B because too many slots fell back to the parent paragraph (signals chronic per-slot budget exhaustion). Propagates as `avg_excessive_parent_fallback_abort_rate`. |
| `prior_picks_sanitization_count` | count | during_execution | (debug_performance_paragraph_recombine_20260612) Number of times `sanitizeForPriorContext` REDACTED an untrusted delimiter-mirror tag inside a previously-chosen paragraph before injecting it into the next round's PRIOR CONTEXT block. Propagates as `total_prior_picks_sanitization_count`. |
| `prior_picks_truncation_count` | count | during_execution | (debug_performance_paragraph_recombine_20260612) Number of times `buildSequentialRewritePrompt` truncated the PRIOR CONTEXT block to satisfy `PRIOR_PICKS_MAX_CHARS=32000` or the 6-prior-paragraph cap. Propagates as `total_prior_picks_truncation_count`. |
