...
| File | Purpose |
|------|---------|
| `evolution/src/lib/treeOfThought/types.ts` | TreeNode, RevisionAction, TreeSearchResult, TreeState, BeamSearchConfig |
| `evolution/src/lib/treeOfThought/treeNode.ts` | Tree construction/traversal: createRootNode, createChildNode, getAncestors, getPath, getBestLeaf, pruneSubtree |
| `evolution/src/lib/treeOfThought/beamSearch.ts` | Core beam search with hybrid two-stage evaluation, re-critique, budget handling |
| `evolution/src/lib/treeOfThought/revisionActions.ts` | Action selection from critiques (forced diversity), per-action-type prompt construction |
| `evolution/src/lib/treeOfThought/evaluator.ts` | Stage 1 (parent-relative filter), Stage 2 (sibling mini-tournament with local OpenSkill ratings) |
| `evolution/src/lib/treeOfThought/index.ts` | Barrel exports |
| `evolution/src/lib/agents/treeSearchAgent.ts` | AgentBase implementation: root selection, budget reservation, pool injection |
| `evolution/src/components/evolution/tabs/LineageTab.tsx` | Tree search visualization (merged into Lineage tab with Full DAG / Pruned Tree toggle) |
