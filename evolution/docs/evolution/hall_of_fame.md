...
Persistent cross-method comparison system using OpenSkill (Weng-Lin Bayesian) ratings, prompt bank, and bias-mitigated pairwise comparisons to rank articles across generation methods.

**Note:** This doc covers the **cross-run** OpenSkill rating system. For the **within-run** OpenSkill rating system, see [Rating & Comparison](./rating_and_comparison.md).

## Overview

The comparison infrastructure extends the evolution pipeline with a persistent **Hall of Fame** system for comparing text quality across different generation methods (1-shot LLM generation, evolution pipeline winners, and evolution baselines). It uses an OpenSkill (Weng-Lin Bayesian) rating system with bias-mitigated pairwise comparisons to rank articles, and provides cost-efficiency metrics (`elo_per_dollar`) to answer the question: "Does spending more on generation produce better quality?"
...
A persistent store organized by **topics** (unique prompts). Each topic contains **entries** — articles generated by different methods/models. Entries are ranked via OpenSkill ratings computed from head-to-head LLM-judged comparisons.
...
The Hall of Fame uses OpenSkill (Weng-Lin Bayesian) ratings, the same algorithm used within pipeline runs (see [Rating & Comparison](./rating_and_comparison.md)):

- Each entry has a `{mu, sigma}` pair: `mu` is estimated skill, `sigma` is uncertainty. New entries start at `mu=25, sigma=8.333`.
...
| Action | Purpose |
|--------|---------|
| `addToHallOfFameAction` | Upsert topic via `upsertTopicByPrompt` (retry on unique constraint race) + entry insert + rating initialization |
...
| `getHallOfFameLeaderboardAction` | Skill-ranked entries joined with entry details |
| `runHallOfFameComparisonAction` | Swiss-style pairwise comparison with configurable rounds, updates OpenSkill ratings |
...
- Topics table with entry counts, rating ranges, costs, best method badges
...
- Skill rating leaderboard with expandable inline rows showing generation metadata
...
User enters prompt + selects model in UI
  → generateAndAddToHallOfFameAction
    ...
    → Initialize OpenSkill rating (mu=25, sigma=8.333, elo_rating derived via ordinalToEloScale)
```
...
Admin clicks "Run Comparison" on topic detail (dialog shows estimated pair count)
  → runHallOfFameComparisonAction(topicId, judgeModel, rounds)
    → Fetch all active entries + current OpenSkill state (mu, sigma, ordinal)
    → For each round:
      → Sort entries by ordinal (descending)
      ...
      → For each pair:
        ...
        → Update in-memory OpenSkill ratings (decisive if confidence >= 0.6, else draw)
    → Persist updated ratings + derived elo_rating to DB
```
...
run-prompt-bank-comparisons.ts → for each topic with ≥ min-entries:
  → fetch entries → all-pairs × rounds → OpenSkill rating updates → persist
  → aggregate summary: avg rating, win rate per method label
```
...
- **Method summary table**: sortable by avg rating, cost, elo/$, win rate; gold highlighting for best values
...
| `evolution/scripts/run-hall-of-fame-comparison.test.ts` | Unit | OpenSkill rating math, CLI arg parsing, comparison flow |
...
- [Rating & Comparison](./rating_and_comparison.md) — OpenSkill rating system (within-run and Hall of Fame share the same algorithm)
