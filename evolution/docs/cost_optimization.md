...
### Mode B per-span approver granularity (default since `investigate_iterative_editing_runs_stage_20260623`)

`coalesceAdjacentGroups` + `capGroupsByMagnitude(K=10)` are now DEFAULT-OFF; every diff atomic the rewrite produces becomes its own singleton approver decision. With the new aggressive proposer prompt removing the `editingProposerSoftCap`-mediated edit-budget, the proposer typically emits 40-60 atomics per cycle, clamped to 30 by `AGENT_MAX_ATOMIC_EDITS_PER_CYCLE`. On `gemini-2.5-flash-lite` the per-cycle approver call costs ~$0.0011 (~30 groups, was ~$0.0006 with K=10 bundling), so per-run cost shifts ~$0.038 → ~$0.041 — well below the typical $0.05 per-run cap. The legacy `disableApproverFiltering` schema field that used to opt OUT of bundling is gone; existing strategy rows that still carry it are silent-tolerated (Zod default strip-unknown).
...
> **Canonical evolution-spend merge (llm_costs_too_low_in_dash_20260623).** Because `llmCallTracking` is incomplete for evolution, the dashboard now sources evolution spend at **invocation grain** from `evolution_agent_invocations` via a new `get_evolution_spend_buckets(p_granularity, p_start, p_end)` RPC (test/real via `run → strategy.is_test_content`), merged app-side with non-evolution `llmCallTracking` (dedup = `call_source NOT LIKE 'evolution_%'`). `getEvolutionReconciliationAction`'s invocation side and the dashboard summary share one helper (`evolutionSpendTotal`) → exact parity. **Default ON** (`COST_DASHBOARD_UNIFIED_EVOLUTION=false` disables) with an auto **prod guard** (falls back on the prod project until the migration is promoted there; `fetchEvolutionSpendBuckets` also degrades to empty on a missing-RPC error). When merged the totals are correct so the dashboard shows **no banner**; the amber "under-counted here" warning shows only when off + materially under-counting. Index `idx_invocations_created_at` backs the windowed scan. **Write-path forward fix:** the runner's inline tracked provider was extracted to `createTrackedEvolutionProvider` and reused by `run-evolution-local.ts` (its direct-SDK `createDirectLLMProvider`, which wrote unjoinable `evolution_v2_*` rows, was deleted + de-allowlisted); real-LLM local runs now route through `callLLM` fail-closed (`--mock` is DB-less). Kill-switch `LLM_REQUIRE_TRACKING_DISABLED`. Scheduled `evolution-tracking-reconciliation.yml` files a `[release-health]` issue on future divergence.
