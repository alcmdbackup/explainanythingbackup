...
> **Two callers, two verdict-tail contracts** (added by `evalute_implied_rubric_results_and_experimentally_validate_20260623` Phase 1). `buildComparisonPrompt` now has a 9th param `strictVerdictTail?: boolean` (default false). When `customPromptOverride` is set, the sandbox builder emits the **reasoning-tolerant** "Your answer:" tail by default — that's the rejudge sandbox's contract, paired with `parseVerdictFromReasoning`. The weight-inference auto-mode caller (`judgePairOnce`) passes `strictVerdictTail: true`, which emits the **strict** "Respond with ONLY one of A/B/TIE" tail — paired with `parseWinner` (start-anchored). So the same `customPromptOverride` plumbing now supports two distinct parser contracts: one tolerant of reasoning (rejudge sandbox), one strict single-token (auto-mode override). The default `strictVerdictTail = false` keeps the rejudge sandbox byte-identical to pre-Phase-1. See [Implicit Rubric Weights → Per-session holistic prompt override](./implicit_rubric_weights.md#per-session-holistic-prompt-override-auto-mode--evalute_implied_rubric_results_and_experimentally_validate_20260623).
