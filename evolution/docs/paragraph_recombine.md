...
2. **For each slot in parallel** (per D18 — `Promise.allSettled`):
   ...
   - **SEQUENTIAL pairwise ranking within the slot** (per D18 — `rankNewVariant` mutates local maps; concurrent calls would corrupt). Each surviving rewrite is binary-search-ranked via `rankNewVariant` against the slot's local pool. The agent relabels `rankNewVariant`'s internal `'ranking'` LLM calls to the dedicated `'paragraph_rank'` AgentName via a thin LLM-client proxy, so per-slot ranking spend buckets into `paragraph_recombine_cost` instead of polluting the article-level `ranking_cost`.
