...
| `judgeModel`        | LLM used for pairwise comparison/judging. Default: `qwen-2.5-7b-instruct` (see `DEFAULT_JUDGE_MODEL` in `src/config/modelRegistry.ts`). Selected based on empirical judge-agreement research (see `docs/analysis/judge_agreement_summary_tables.md`) — 100% decisive on both large-gap and close-pair comparisons with ~1.7s median latency and no thinking-mode overhead. |
...
- **listPublicStrategiesAction** (Phase 1, `publicAction`-wrapped — NOT admin-gated) -- returns the curated subset for the `/edit` picker. Filter shape lives in the shared `evolution/src/services/publicStrategyFilter.ts` (single source of truth reused by `submitPublicEditAction`). Post-`improvements_to_edit_page_evolution_20260630`: gated on env `PUBLIC_EDIT_WIDEN_FILTER`. When `'true'` returns all `status='active' AND is_test_content=false AND config.generationModel != 'mock'` strategies. When unset/`'false'` (default) additionally requires `public_visible=true` — matches legacy behavior. `PublicStrategySummary` now includes `budgetUsd` (drives the picker's ⚠ warning badge when > $0.10). 60s in-memory cache (per serverless instance); invalidated by `updateStrategyAction`, `archiveStrategyAction`, and `deleteStrategyAction`. A sibling `getPublicStrategyConfigAction(strategyId)` returns the full `StrategyConfig` for the picker's *Show config* modal (re-applies the same filter as a safety re-check).
- **PUBLIC_VISIBLE_BUDGET_CAP_USD** ($0.10) — **DEPRECATED** post-`improvements_to_edit_page_evolution_20260630`. `updateStrategyAction`'s per-strategy publicVisible guard is now vestigial: the widened filter no longer keys on `public_visible`, so the guard doesn't prevent any picker outcome. `PublicVisibleToggle.tsx` + the guard + the constant are slated for cleanup in a follow-up PR. Per-run cost is now capped by `evolution_runs.budget_cap_usd = strategy.config.budgetUsd` (Zod-validated `> 0`, `<= $10` at submit-time) — see `docs/feature_deep_dives/llm_spending_gate.md`.
