...
| **Flow Guardrail** | Transition-word regex rule that drops proposed edits whose `oldText` matches `RE_TRANSITION_START` (`/^(However\|Therefore\|Thus\|Moreover\|...)/i`) at a paragraph start AND whose `newText` doesn't preserve the transition. Maintains paragraph-to-paragraph connective tissue. Lives in `proposer_approver_criteria_generate`'s `validateEditGroups` opts (removed at the coherence-pass call site by `investigate_paragraph_recombine_coherence_pass_performance_20260623` Phase 2a). |
