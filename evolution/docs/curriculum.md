...
| **Criteria** | A user-defined evaluation dimension (e.g., `clarity`, `engagement`, `depth`) used by all 3 criteria-driven agent types to score a parent article and target the K weakest dimensions for revision. Stored in `evolution_criteria` (DB-first, soft-delete via `deleted_at`); each row carries an optional rubric (`evaluation_guidance`) of `{score, description}` anchors. Selected per-iteration via `agentType ∈ {criteria_and_generate, single_pass_evaluate_criteria_and_generate, proposer_approver_criteria_generate}` + `criteriaIds` + `weakestK` on `IterationConfig`. See [Agents Overview](./agents/overview.md#evaluatecriteriathengeneratefrompreviousarticleagent-evaluatecriteriathengeneratefrompreviousarticle_20260501) and [Criteria Agents Deep Dive](./criteria_agents.md). |
| **Mirror Approver** | Bias-mitigation pass used by `proposer_approver_criteria_generate`. The approver runs twice on each proposed edit group: a forward pass on the original proposal, and a mirror pass on a sign-flipped version applied to the post-apply article. The aggregator applies the edit iff `(forward, mirror) === ('accept', 'reject')` — the approver consistently prefers the proposed end-state in both framings. Strict binary; everything else drops. Mirrors the existing `run2PassReversal` bias-mitigation pattern used for pairwise judges. |
| **Redundancy Guardrail** | Trigram-Jaccard semantic-overlap check (`checkSemanticOverlap`) that rejects proposed edits whose `newText` shares more than `redundancyJaccardThreshold` (default 0.35) of word-trigrams with the rest of the article. Lexical, not semantic — catches verbatim restatements; misses paraphrased duplication. Lives in the new criteria agents' `validateEditGroups` opts. |
| **Flow Guardrail** | Transition-word regex rule that drops proposed edits whose `oldText` matches `RE_TRANSITION_START` (`/^(However\|Therefore\|Thus\|Moreover\|...)/i`) at a paragraph start AND whose `newText` doesn't preserve the transition. Maintains paragraph-to-paragraph connective tissue. Lives in the new criteria agents' `validateEditGroups` opts. |
| **Length Cap Ratio** | Tightened size-ratio cap on `proposer_approver_criteria_generate` edits (default 1.10 — ±10%). Edits whose projected post-apply length exceeds `currentText.length * lengthCapRatio` get dropped. Looser default (1.50) for `iterative_editing` callers via the same `validateEditGroups` parameterization. |
