...
| `source` | TEXT | NOT NULL, DEFAULT `'admin'`, CHECK ∈ (`'admin'`,`'public_edit'`,`'test'`,`'local'`,`'minicomputer'`) | Migration `20260629000001`. Denormalizes `evolution_runs.run_source` for the topic origin so arena topic-list filters don't need a 3-table JOIN. Set on INSERT by `upsertSlotTopic` from the agent's `ctx.runSource` (threaded from `claim_evolution_run` → `claimAndExecuteRun` → `evolveArticle` → AgentContext). Preserved on ON-CONFLICT path so admin-created topics stay `'admin'`. Backed by partial index `idx_evolution_prompts_source_non_admin` for the steady-state default filter `source != 'public_edit'`. `getArenaTopicsAction` hides `source='public_edit'` rows unless `includePublicEdit=true` (UI toggle `"Show /edit submissions"`, default off). |
...
- `evolution_weight_inference_sessions` — run entity: `id`, `name` UNIQUE, `description`, `status` (`active`/`archived`), **`mode`** (`human`/`auto`), `prompt_id` (arena topic, bare UUID), `sample_size`, `replication_rate` (reversal-audit fraction), auto-mode `judge_model`/`judge_temperature`/`judge_reasoning_effort`/`auto_repeats`/`auto_run_error`, **`holistic_prompt_override`** (`TEXT NULL`, ≤8000 chars, Zod-deny-listed for `## Text A`/`## Text B`/`Your answer:`/`<|`/`|>`; auto-mode-only override for `buildComparisonPrompt`'s holistic checklist — see [Implicit Rubric Weights → Per-session holistic prompt override](./implicit_rubric_weights.md#per-session-holistic-prompt-override-auto-mode--evalute_implied_rubric_results_and_experimentally_validate_20260623)), `is_test_content`, soft-delete, timestamps.
...
| `20260620000001_evolution_weight_inference_test_set_source.sql` | 2026-06-20 | Phase 6: adds `evolution_weight_inference_sessions.source_kind` (`topic`/`test_set`), `judge_eval_test_set_id`, and `pair_kind` (`article`/`paragraph`) so a session can source its pairs from a Judge Lab test set and judge article OR paragraph pairs. Additive columns + CHECK constraints. |
| `20260624173001_evolution_weight_inference_holistic_override.sql` | 2026-06-23 | evalute_implied_rubric_results_and_experimentally_validate_20260623 Phase 1: adds `evolution_weight_inference_sessions.holistic_prompt_override TEXT NULL` (≤ 8000-char CHECK). Auto-mode-only override for the holistic comparison prompt's hardcoded checklist; default NULL preserves byte-identical legacy behavior. Per-criterion rubric prompt is unaffected. See [Implicit Rubric Weights → Per-session holistic prompt override](./implicit_rubric_weights.md#per-session-holistic-prompt-override-auto-mode--evalute_implied_rubric_results_and_experimentally_validate_20260623). |
