...
| `EVOLUTION_USE_SCOPE_OWNSPENT` | `true` | When `true` (default), `Agent.run()` uses `costScope.getOwnSpent()` as the authoritative cost source for per-invocation `cost_usd`. Set to `'false'` to fall back to the legacy `detail.totalCost`-delta path (used before Phase 2.5 fixed sibling-cost bleed under parallel dispatch). Flip via Vercel env without redeploying to roll back if the scope-routed client regresses. Rollback window: one week post-deploy, then fallback path is removed. |
...
Maximum 3 retries. Per-call timeout is 60 seconds. Budget is reserved before each attempt and released on failure, so retries do not double-count cost. Non-transient errors (auth failures, content policy, budget exceeded) are thrown immediately without retry.
