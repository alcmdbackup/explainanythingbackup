...
| `EVOLUTION_PARAGRAPH_RECOMBINE_REPLAN_ENABLED` | `false` | Mid-sequence coordinator replan (investigate_sequential_paragraph_recombine_performance_20260615 Phase 2 / Fix 2). Default `'false'`: byte-identical to today. `'true'`: after slot 0 finalizes AND the 6-condition success predicate holds, the coordinator is called again with `priorPicks + firstSlot=1` so slots 1..N-1's directives can match the chosen opener voice. Adds ~$0.0014 per invocation via the separate `paragraph_recombine_coordinator_replan` LLM label. Auto-disabled when `perInvocationCapUsd < REPLAN_MIN_CAP_USD = 0.030`. See `evolution/docs/paragraph_recombine.md` Sequential perf tuning section. |
