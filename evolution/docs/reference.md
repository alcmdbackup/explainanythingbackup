...
| `LLM_GATE_PANIC_BYPASS` | unset | Phase 0. When `'true'`, ALL gate checks short-circuit + audit-log to stderr per call. Last-resort. (The original Phase-0 `LLM_GATE_FAIL_CLOSED_DISABLED` rollback flag was removed after staging soak — fail-CLOSED is now unconditional.) |
