...
| `EVOLUTION_REFLECTION_ENABLED` | `'true'` | Ignores `iterCfg.useReflection: true` and dispatches vanilla `GenerateFromPreviousArticleAgent` for every generate iteration. Single env flip rolls reflection back without code revert. Resolved once per iteration in `runIterationLoop` before parallel/top-up dispatch. | develop_reflection_and_generateFromParentArticle_agent_evolution_20260430 |
