...
| `EVOLUTION_MAX_OUTPUT_TOKENS` | `4096` | D5 (fix_structured_judging_evolution_bugs_20260611). Output-token cap forwarded as `CallLLMOptions.maxOutputTokens` → OpenAI `max_tokens` for every evolution LLM call (set at the `claimAndExecuteRun` `complete()` chokepoint). Applied by `callOpenAIModel` only to **non-reasoning** models (reasoning models exempted — the cap would truncate the trace). Prevents OpenRouter's affordability pre-check from reserving the model max (~65535) and 402-ing on a low balance. Set to a large value to neutralize a truncation regression without a redeploy (runner restart required). |
...
| `EVOLUTION_PARAGRAPH_RECOMBINE_COHERENCE_ENABLED` | `true` | Kill-switch for `paragraph_recombine_with_coherence_pass` iterations (paragraph_recombine_agent_with_coherence_pass_evolution_20260620). When `'false'`, the dispatch loop short-circuits to `iteration_complete` with zero variants. Single-env-flip rollback. See [paragraph_recombine_with_coherence_pass.md](./paragraph_recombine_with_coherence_pass.md) for the agent's full design. |
...
| `EVOLUTION_COHERENCE_PASS_DEFAULTS_V2` | `true` | Kill-switch for the new aggressive coherence-pass defaults (investigate_paragraph_recombine_coherence_pass_performance_20260623). Default `'true'` (or unset): `coherencePassLengthCapRatio=1.10`, `coherencePassMaxCycles=2`. Setting `'false'` reverts to legacy defaults (`1.02`, `1`) WITHOUT a deploy — `resolveCoherencePassDefaults()` reads the env var per-invocation. Explicit per-strategy iter-config values ALWAYS override; this only changes what the DEFAULT is. |
...
> **402 is non-transient.** `classifyErrors.ts` matches only 429/408/5xx as transient, so an OpenRouter **402** (out of credits / max_tokens-too-high) is thrown immediately — `release()` not `recordSpend()`, so cost stays 0. A 402 during generation surfaces as a generate-agent hard failure → invocation `success=false` (D1) → `error_code='all_generations_failed'` at finalize when it wipes out the whole run (D3). A 402 during **ranking** is still swallowed as a confidence-0 TIE (`rankSingleVariant.ts`) — an un-fixed sibling path (follow-up). Mitigate the trigger with the `EVOLUTION_MAX_OUTPUT_TOKENS` cap (D5).
