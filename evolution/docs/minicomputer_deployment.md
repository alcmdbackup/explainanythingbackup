...
6. **Phase 0 of `build_website_for_evolutiOn_20260626`:** the runner calls `cleanup_orphaned_per_user_reservations(15)` against each target BEFORE the while-loop. Placed pre-loop so it fires once per systemd-timer wake even on empty-queue invocations (which exit at "No pending runs found"). Best-effort — swallows per-target errors so a transient DB blip doesn't abort the runner.
7. Round-robin loop iterates targets, calling `claimAndExecuteRun({ runnerId, db: target.client })` for each. Up to `--parallel N` runs execute concurrently via `Promise.allSettled`
8. `claimAndExecuteRun` handles the full lifecycle per run: claim → heartbeat → pipeline → finalize → cleanup
9. When no targets have pending runs, the process exits. Systemd starts it again on the next timer tick
