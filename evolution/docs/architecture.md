...
| **Paragraph recombine** | 1 `ParagraphRecombineAgent` — decomposes the parent into N paragraph slots, generates M parallel rewrites per slot, ranks per-slot, recombines winners into 1 article variant. Per-slot matches persist via `persistSlotMatches` (NOT MergeRatingsAgent — slot topics are independent of article-pool ratings). | None at article level (per-slot ranking is inline) | Format validation rejection / per-slot budget exhaustion → fall back to original |
