...
| **Paragraph recombine** | 1 `ParagraphRecombineAgent` when `maxDispatches=1` (default — back-compat), OR **K parallel ParagraphRecombineAgent invocations** with sequential top-up when `maxDispatches>1` + `sourceMode='pool'` (J of `investigate_paragraph_rewrite_cost_undershoot_evolution_20260529`). Each invocation decomposes its parent into N paragraph slots and produces 1 article variant. **Two execution modes** controlled by `EVOLUTION_PARAGRAPH_RECOMBINE_SEQUENTIAL_ENABLED` (default `true`, debug_performance_paragraph_recombine_20260612): (a) **Sequential Context-Aware Generation** — one coordinator LLM call up-front returns per-paragraph plans with M strategically-diverse directives; then a sequential per-paragraph loop where each round reads PRIOR CONTEXT (chosen paragraphs 0..i-1, untrusted-tagged) + ORIGINAL PARAGRAPH i, generates M candidates per coordinator's directives, ranks via existing Elo infrastructure (judge optionally also sees prior picks), and appends the winner to `priorPicks`. (b) **Legacy parallel** — N slots dispatched concurrently via `Promise.allSettled`, no inter-paragraph context. Per-slot matches persist via `persistSlotMatches` in both modes (slot topics are independent of article-pool ratings). | **1 `MergeRatingsAgent`** over ALL K invocations' concatenated match buffers — the agents article-rank their recombined variants via `rankNewVariant` and return the matches; the branch merges them so the variants compete for the run winner. (Per-slot ranking is separate + inline.) | Format validation rejection / per-slot budget exhaustion → fall back to original. Sequential mode: B.7 mid-loop throw persists truncated `slots[0..i-1]` + `partialAt` + `abortReason` + `completedSlotCount` before rethrow. |
