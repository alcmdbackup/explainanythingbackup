...
| **Paragraph recombine** | 1 `ParagraphRecombineAgent` when `maxDispatches=1` (default — back-compat), OR **K parallel ParagraphRecombineAgent invocations** with sequential top-up when `maxDispatches>1` + `sourceMode='pool'` (J of `investigate_paragraph_rewrite_cost_undershoot_evolution_20260529`). Each invocation decomposes its parent into N paragraph slots, generates M parallel rewrites per slot, ranks per-slot, recombines winners into 1 article variant. Per-slot matches persist via `persistSlotMatches` (slot topics are independent of article-pool ratings). Multi-dispatch mirrors the generate-iteration RUNTIME pattern: parallel batch sized by `min(DISPATCH_SAFETY_CAP, floor(availBudget/projector.expected), maxDispatches, eligibleParents.length)` with NO `resolveParallelFloor` at runtime (projector-only); sequential top-up gated by `resolveSequentialFloor`. Distinct-parent enforcement via seeded pre-shuffle. | **1 `MergeRatingsAgent`** over ALL K invocations' concatenated match buffers — the agents article-rank their recombined variants via `rankNewVariant` and return the matches; the branch merges them so the variants compete for the run winner. (Per-slot ranking is separate + inline.) | Format validation rejection / per-slot budget exhaustion → fall back to original |
