...
| `/admin/evolution/judge-lab` (+ `/runs/[evalRunId]`, `/pair-banks`, `/test-sets`) | **Judge Lab** (Tools nav group; create_tool_systematic_judge_evaluation_evolutioN_20260606) — batch + persisted judge-settings evaluation, the measurement layer atop the Match Viewer's display-only re-judge. Launcher picks a frozen **Test Set** + Kind (article/paragraph/both) + models × temperatures × reasoning + optional custom rubric → cost-capped sweep (`createEvalRunAction`, hard `JUDGE_EVAL_MAX_CALLS`/`MAX_USD` ceiling before any LLM call). **Leaderboard** (`judge_eval_settings_leaderboard` VIEW) ranks settings by decisive rate, scoped to the test set, split Article/Paragraph. Run detail shows per-kind aggregates + per-pair breakdown (metrics computed via the same pure reducer as the CLI). Pair-banks/test-sets managers seed from an arena topic + carve frozen samples. See [Judge Evaluation deep dive](../../docs/feature_deep_dives/judge_evaluation.md). | Decisive-rate leaderboard, per-kind metrics, frozen test sets |
