...
| `/admin/evolution/arena/[topicId]` | Topic leaderboard sorted by Elo rating. Columns: Elo, 95% CI (formatted via `formatEloCIRange(elo, sigma)`), Mu, Sigma, Matches, Cost. Entries below the top 15% eligibility cutoff (mean + 1.04×stdDev of Elo scores) are dimmed. Cutoff logic is in `src/app/admin/evolution/arena/[topicId]/arenaCutoff.ts`. Entries in the bottom 25th percentile of sigma display a gold "Anchor" badge, and the leaderboard header shows the total anchor count. | TrueSkill ratings, match history, anchor badges |
