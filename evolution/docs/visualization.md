...
| `/admin/evolution/runs/[runId]` | Run detail with tabs: **Timeline**, **Metrics**, **Cost Estimates**, **Elo**, **Lineage**, **Variants**, **Snapshots**, **Logs**. Auto-refreshes while run is in progress. **Timeline tab** shows a Gantt-style view of agent invocations grouped by iteration. **Cost Estimates tab** renders summary MetricGrid, Cost-by-Agent breakdown (makes the estimation-coverage gap explicit), conditional Budget Floor Sensitivity (hidden when sequential floor isn't AgentMultiple mode or parallel had no successful agents), inline-SVG error histogram, and a sortable Cost-per-Invocation table; powered by `getRunCostEstimatesAction` in `evolution/src/services/costEstimationActions.ts`. | Full run metrics, Gantt timeline, cost-estimate breakdown, lineage graph, variant list |
...
| `/admin/evolution/strategies/[strategyId]` | Strategy detail with tabs: **Metrics**, **Cost Estimates**, **Runs**, **Configuration**, **Logs**. The **Runs** tab shows runs filtered by `strategy_id`. **Cost Estimates tab** renders a propagated summary (total/avg cost, avg estimation error %, runs-with-estimates count), a per-(strategy × generation_model × judge_model) slice breakdown capped at 50 rows, an error-distribution histogram, and a Runs table with drill-down links to each run's Cost Estimates tab; powered by `getStrategyCostEstimatesAction`. | Strategy config, linked runs, cost-estimate breakdown, aggregated logs across all runs using this strategy |
