...
| `/admin/evolution/strategies/[strategyId]` | Strategy detail with tabs: **Metrics**, **Cost Estimates**, **Runs**, **Variants**, **Configuration**, **Logs**. The **Runs** tab shows runs filtered by `strategy_id`. The **Variants** tab (Phase 4c) renders all variants across all runs of this strategy via `<VariantsTab strategyId=... />` — same component as the run Variants tab but parameterized with `strategyId` instead of `runId`. **Cost Estimates tab** renders a propagated summary (total/avg cost, avg estimation error %, runs-with-estimates count), a per-(strategy × generation_model × judge_model) slice breakdown capped at 50 rows, an error-distribution histogram, and a Runs table with drill-down links to each run's Cost Estimates tab; powered by `getStrategyCostEstimatesAction`. | Strategy config, linked runs, variants across runs with Elo ± uncertainty + 95% CI, cost-estimate breakdown, aggregated logs across all runs using this strategy |
