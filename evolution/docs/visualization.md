...
| `/admin/evolution/variants/[variantId]` | Variant detail with full prompt text, metrics, lineage context, and a **Matches** tab populated from `getVariantMatchHistoryAction` (which queries `evolution_arena_comparisons` via `.or('entry_a.eq.<id>,entry_b.eq.<id>')`; previously a stub returning `[]`, fixed in fixes_to_evolution_admin_dashboard__20260503). Header carries a "Produced by `<agent_name>`" cross-link to `/admin/evolution/invocations/[id]` when `agent_invocation_id` is populated (~55% of variants per staging — pre-2026-04-18 variants have NULL FK and the link gracefully omits). The red "Discarded variant" banner (`data-testid="variant-discarded-banner"`) renders only for **article** variants with `persisted=false` (generate-agent discards). Paragraph snippets are always `persisted=false` by design (`sync_to_arena`) and are NOT shown as discarded — gated by `isDiscardedGenerateVariant` (`evolution/src/lib/utils/variantStatus.ts`); the same gate applies to VariantsTab/LineageGraph/SnapshotsTab, and the default list filters keep paragraph variants via `NON_DISCARDED_OR_FILTER` (investigate_banner_on_paragraph_rewrite_paragraph_variant_20260531). | Prompt content, parent chain, comparison results, match history, producing invocation |
