...
| `/admin/evolution/weight-inference` (+ `/[sessionId]`) | **Implied Rubric Weights** (Tools nav group; calculate_implifed_rubric_weights_evolution_20260619) — infer judge-rubric weights from pairwise verdicts. Landing: sessions list + new-session form with a **pair source** toggle — an **arena topic** (sampled article variants) or a **Judge Lab test set** (frozen pairs, with an article/paragraph **pair-kind** toggle) — plus a criteria multi-select (descriptions on hover + a selected-criteria description list), pool size + reversal-audit rate, and a live ratings-needed preview. Session detail: **Judge** tab (OVERALL verdict first, then a gated **per-criterion** step on the same pair) + **Results** tab (weight bars + bootstrap CIs + barely-matters/disagrees flags + reviewer-bias audit + **Export as judge rubric** → a real `evolution_judge_rubrics` row). Human mode; auto mode (LLM-as-judge) adds a mode toggle + Run tab. Server actions in `weightInferenceActions.ts`; kill switch `EVOLUTION_WEIGHT_INFERENCE_ENABLED`. See [Implicit Rubric Weights](./implicit_rubric_weights.md). | Inferred weights + CIs, position-bias/self-consistency, exported rubric |
