...
**Per-cycle protocol**:
1. **Proposer** (`iterative_edit_propose`) — LLM call. Output is the FULL ARTICLE BODY VERBATIM with inline CriticMarkup edits: `{++ inserted ++}`, `{-- deleted --}`, `{~~ old ~> new ~~}`, plus the standard CriticMarkup paired form `{~~ old ~~}{++ new ++}` for substitutions. The optional `[#N]` group tag (e.g. `{++ [#1] inserted ++}`) forces grouping across non-adjacent spans; if omitted, **each unnumbered span is its OWN group** — the reviewer gets one decision per atomic edit (per-span granularity, post-`investigate_iterative_editing_runs_stage_20260623`). The standard CriticMarkup paired form `{~~ X ~~}{++ Y ++}` is treated as ONE substitution edit via a position-adjacency rule (delete-immediately-followed-by-insert, only horizontal whitespace between spans, NO newlines) — independent of group numbers. Mode B's post-parse `coalesceAdjacentGroups` + `capGroupsByMagnitude(K=10)` are now DEFAULT-OFF: every diff atomic the rewrite produces becomes its own approver decision.
