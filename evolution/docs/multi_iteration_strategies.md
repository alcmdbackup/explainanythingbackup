...
const iterationConfigSchema = z.object({
  ...
  coherencePassLengthCapRatio: z.number().min(1.0).max(2.0).optional(),  // default 1.10
  coherencePassMaxCycles: z.number().int().min(1).max(5).optional(),     // default 2
  // rebuild_coherence_pass_agent_mode_ab_configurable_20260624
  coherencePassEditingMode: z.enum(['mode_a', 'mode_b']).optional(),     // default 'mode_b'
...
- `lengthCapRatio` is rejected on agent types other than `proposer_approver_criteria_generate`. `includesMirrorApprover` is rejected on agent types other than `proposer_approver_criteria_generate`. The `editingMaxCycles === 1` invariant is enforced for `proposer_approver_criteria_generate`. `coherencePassLengthCapRatio`, `coherencePassMaxCycles`, and `coherencePassEditingMode` are rejected on agent types other than `paragraph_recombine_with_coherence_pass`.
