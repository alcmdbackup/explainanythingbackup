...
- [x] Define `IterationConfig` Zod schema in `evolution/src/lib/schemas.ts`:
  ```typescript
...
- [x] Add Zod refinement: `maxAgents` must be undefined for swiss iterations (`.refine(c => c.agentType !== 'swiss' || c.maxAgents === undefined)`)
- [x] Extend `strategyConfigSchema` with `iterationConfigs: z.array(iterationConfigSchema).min(1)` — replaces the flat `iterations: number` field
- [x] Remove `maxVariantsToGenerateFromSeedArticle` from strategy-level schema — replaced by `maxAgents` on generate iteration configs
- [x] Remove `iterations: number` from strategy-level schema — replaced by `iterationConfigs.length`
- [x] Add Zod refinement: sum of `iterationConfigs[].budgetPercent` must equal 100 (use tolerance: `Math.abs(sum - 100) < 0.01` for floating-point safety; "Split evenly" button handles remainder distribution to last iteration)
- [x] Add Zod refinement: first iteration must be `agentType: 'generate'` (swiss on empty pool is invalid)
- [x] Add Zod refinement: no swiss iteration may precede all generate iterations
- [x] Strategy-level `budgetUsd` remains as the total budget; per-iteration dollar amounts computed at runtime as `budgetPercent / 100 * budgetUsd`
- [x] **Seed cost accounting**: Seed generation cost (typically ~$0.01-0.05) is charged to the run-level budget before iterations begin. This means the effective run budget available to iterations is `budgetUsd - seedCost`. The last iteration may be slightly short-changed by seed cost. This is acceptable — the run-level tracker is the safety net, and seed cost is small relative to total budget. If seed cost is significant (>5% of budget), the orchestrator logs a warning. No percentage adjustment needed — iterations still compute dollar amounts from full `budgetUsd`, and the run-level tracker catches any overrun.
- [x] Update `hashStrategyConfig()` — note: the hash function lives in `evolution/src/lib/shared/hashStrategyConfig.ts` (with `StrategyHashInput` interface) and is re-exported by `findOrCreateStrategy.ts`. Update `StrategyHashInput` to include `iterationConfigs` and update the hash computation to serialize the full array. Both files and their tests (`findOrCreateStrategy.test.ts`, `hashStrategyConfig.test.ts` if exists) must be updated.
- [x] Update `labelStrategyConfig()` to reflect iteration count and types (e.g., "2×gen + 3×swiss")
- [x] Update `evolutionConfigSchema` in `evolution/src/lib/schemas.ts` to add `iterationConfigs` field (required array). Remove deprecated `iterations` field. Keep `numVariants` as deprecated (no longer used — `maxAgents` on iteration config replaces it). This is a **Phase 1 prerequisite for Phase 4** — the orchestrator reads `config.iterationConfigs` which must exist on EvolutionConfig.
- [x] Extend `EvolutionConfig` type in `evolution/src/lib/pipeline/infra/types.ts` — type is inferred from the updated schema via `z.infer<>`
- [x] Update `buildRunContext.ts` (lines 246-264) to map `strategyConfig.iterationConfigs` → `EvolutionConfig.iterationConfigs`. Compute `iterations` as `iterationConfigs.length` for any legacy code paths that still read it.
- [x] DB migration: no schema change needed — `iterationConfigs` is stored inside the existing `evolution_strategies.config` JSONB column
- [x] No backward compatibility needed — old strategies without `iterationConfigs` are incompatible. Pipeline rejects them at config validation.
- [x] Add Zod refinement: `iterationConfigs.length` must be ≤ `MAX_ORCHESTRATOR_ITERATIONS` (currently 20) to prevent user-defined configs that exceed the safety cap
- [x] **Fixture migration checklist** — these test files/fixtures use the old `iterations: number` format and must be updated to `iterationConfigs[]`:
...
- [x] In `runIterationLoop.ts` (lines 220-265): stop adding seed variant to `pool[]` — keep only as `originalText` for generation
- [x] Remove seed from `initialPool`, `ratings` map, and `matchCounts` in the loop initialization
- [x] Keep `loadArenaEntries()` behavior (arena entries still enter pool as competitors)
- [x] Move seed generation to pre-iteration setup phase (before the iteration loop begins). This requires refactoring the existing `seedPrompt` code path in `runIterationLoop.ts` (lines 367-418) which currently runs `CreateSeedArticleAgent` inside iteration 1's generate block:
...
- [x] Update `evolveArticle()` function signature: remove `seedPrompt` and `seedVariantRow` from options (they're resolved before the call). Update 7+ test call sites in `runIterationLoop.test.ts` and integration tests.
- [x] Update `persistRunResults.ts`: remove `reusedFromSeed` filtering logic from `summaryPool` / `localPool` — seed is never in pool
- [x] Update run summary construction: remove `seedVariantRank` / `seedVariantElo` from `EvolutionRunSummaryV3` (seed no longer competes, so it has no rank)
- [x] Update `syncToArena()`: remove optimistic-concurrency UPDATE path for reused seed — seed row is managed outside the run
- [x] Update winner determination in `selectWinner.ts`: no change needed (seed already not in pool)
- [x] Set seed variant's `parentIds` to `[]` (it's the root) — seed text is passed as `originalText` to generate agents
- [x] Mark seed in arena: add `is_seed` computed property to `ArenaEntry` interface (check `generation_method === 'seed'`)
- [x] Arena leaderboard UI (`[topicId]/page.tsx`): add visual seed badge using `StatusBadge` component with distinct color
...
- [x] In `persistRunResults.ts` (line 216): change `generation: v.version` → `generation: v.iterationBorn`
- [x] In `generateFromSeedArticle.ts` (line 228-234): set `parentIds: [seedVariantId]` where `seedVariantId` is passed via agent input (the seed's UUID from arena or newly generated)
- [x] Pass `seedVariantId` as part of `GenerateFromSeedInput` (not AgentContext — AgentContext is infrastructure, not domain data). Plumb from `runIterationLoop.ts` where seed is resolved via `originalText`.
- [x] In `createSeedArticle.ts`: seed's own `parentIds` remains `[]` (root variant)
- [x] Verify `iterationBorn` is correctly set on all variant creation paths:
...
- [x] Add `resolveIterationConfig()` function in `runIterationLoop.ts`:
  ```typescript
  ...
  ```
- [x] Replace `nextIteration()` decision function: iterate through `iterationConfigs[]` sequentially (index 0, 1, 2, ...) instead of oracle-driven generate/swiss logic
- [x] Create per-iteration budget tracker: `createIterationBudgetTracker(iterationBudgetUsd, runCostTracker)` — wraps the run-level tracker with an iteration-level cap:
...
- [x] Implement per-iteration stop reasons as a **separate enum** from run-level `stopReason`:
...
- [x] Update dispatch logic: for each iteration config entry:
...
- [x] Pass iteration-specific dispatch params (numVariants, budget) while agents inherit strategy-level models/temperature from `AgentContext.config`
- [x] Record iteration stop reason in `iterationSnapshots` (extend schema with `stopReason` field)
- [x] Record iteration budget state in snapshots (extend schema with `budgetAllocated`, `budgetSpent`)
- [x] Update `EvolutionResult` to include per-iteration results: `iterationResults: Array<{ iteration: number; stopReason: string; budgetSpent: number; variantsCreated: number }>`
- [x] Update config validation at loop entry (lines 32-49): validate `iterationConfigs[]` instead of flat `iterations` field
- [x] Migrate stopReason consumers: grep all pattern matches on old values (`iterations_complete`, `converged`, `no_pairs`, `seed_failed`) in persistRunResults, finalizeRun, UI components, and update to new enum values. `seed_failed` is removed (handled as thrown error in Phase 2).

### Phase 5: Strategy Creation Wizard UI
Split strategy creation from a single FormDialog into a multi-page wizard.

- [x] Create new page: `src/app/admin/evolution/strategies/new/page.tsx` — dedicated strategy creation route
- [x] Create `StrategyForm` component (pattern after `ExperimentForm`) — 2-step wizard:
...
- [x] Extract `BudgetFloorsField` from `strategies/page.tsx` into `evolution/src/components/evolution/fields/` for reuse
- [x] Iteration configuration step UI (Step 2):
...
- [x] Keep existing FormDialog on strategies list page for quick edit of name/description/status only
- [x] Update strategy list page: "Create" button navigates to `/strategies/new` instead of opening FormDialog
- [x] Update `createStrategyAction` in `strategyRegistryActions.ts` to accept and validate `iterationConfigs[]`

### Phase 6: Strategy Detail & Config Display
Update strategy detail page to show per-iteration config and add variants tab.

- [x] Update `StrategyConfigDisplay.tsx`: add "Iterations" section showing a card per iteration with: agent type badge, budget allocation, max agents (generate only), budget floor settings
- [x] Add Variants tab to strategy detail page (`strategies/[strategyId]/page.tsx`):
...
- [x] Update `StrategyEntity.ts` detailTabs to include `variants`

### Phase 7: Run Detail Per-Iteration Visibility
Enhance run detail tabs with per-iteration filtering and display.

- [x] **Timeline tab** (`TimelineTab.tsx`) — redesign as consolidated iteration view:
...
- [x] **Cost Estimates tab** (`CostEstimatesTab.tsx`):
...
- [x] **Variants tab** (`VariantsTab.tsx`):
...
- [x] **Snapshots tab**: already per-iteration — enhance with iteration config display (show what agent type, budget, model was used)
- [x] **Elo tab**: add vertical markers at iteration boundaries on convergence chart

### Phase 8: Arena & Variant Surface Changes
Surface iteration and parent variant info across remaining UI surfaces.

- [x] **Arena leaderboard** (`arena/[topicId]/page.tsx`):
...
- [x] **Variants list page** (`variants/page.tsx`):
  - Add "Parent Variant" column
  - Update `listVariantsAction` to fetch `parent_variant_id`
- [x] **Variant detail page** (`variants/[variantId]/VariantDetailContent.tsx`):
...
### Unit Tests
- [x] `evolution/src/lib/schemas.test.ts` — test iterationConfigSchema validation, refinements:
...
- [x] `evolution/src/lib/pipeline/loop/runIterationLoop.test.ts` — test per-iteration config resolution, iteration budget enforcement, per-iteration stop reasons, seed not in pool
- [x] `evolution/src/lib/pipeline/setup/findOrCreateStrategy.test.ts` — test hash includes iterationConfigs, label generation
- [x] `evolution/src/lib/pipeline/setup/buildRunContext.test.ts` — test iterationConfigs mapping, reject legacy configs without iterationConfigs
- [x] `evolution/src/lib/pipeline/cost-tracker.test.ts` — test iteration-level budget tracker wrapping run-level tracker:
...
- [x] `evolution/src/lib/pipeline/finalize/persistRunResults.test.ts` — test generation stores iterationBorn, parentIds set correctly
- [x] `evolution/src/services/strategyRegistryActions.test.ts` — test createStrategyAction with iterationConfigs

### Integration Tests
- [x] `src/__tests__/integration/evolution-iteration-config.integration.test.ts` — full pipeline run with multi-iteration config: generate → swiss → generate. Use real `createCostTracker` (not mocked) with small budgets to exercise two-layer budget enforcement end-to-end. Verify: iteration budget exhaustion produces `IterationBudgetExceededError` (run continues), run budget exhaustion produces `BudgetExceededError` (run stops), per-iteration stop reasons recorded in `iterationResults[]`.

### E2E Tests
- [x] `src/__tests__/e2e/specs/09-admin/admin-strategy-wizard.spec.ts` — strategy creation wizard: fill base config, add iterations, submit
- [x] Update existing evolution E2E specs (`admin-evolution-run-pipeline.spec.ts`, `admin-evolution-experiment-wizard-e2e.spec.ts`) — verify run detail shows iteration column in variants, cost estimates per-iteration. Note: `admin-evolution-v2.spec.ts` does not exist; use actual spec file names.

### Manual Verification
- [x] Create a strategy with 3 iterations (generate, swiss, generate) via the new wizard
- [x] Run an experiment with that strategy, verify per-iteration budget enforcement
- [x] Verify seed variant appears with badge in arena leaderboard
- [x] Verify variants show iteration number and parent variant link in run detail
- [x] Verify strategy detail page shows per-iteration config and has working variants tab

## Verification

### A) Playwright Verification (required for UI changes)
- [x] Strategy creation wizard — multi-step flow with iteration configuration
- [x] Run detail — variants tab shows iteration and parent columns
- [x] Arena leaderboard — seed variant badge visible
- [x] Strategy detail — per-iteration config display and variants tab

### B) Automated Tests
- [x] `npm run test:unit -- --testPathPattern="evolution"` — all evolution unit tests pass
- [x] `npm run test:integration -- --testPathPattern="evolution"` — integration tests pass
...
- [x] `evolution/docs/architecture.md` — per-iteration config-driven loop, two-layer budget, seed decoupling
- [x] `evolution/docs/arena.md` — seed no longer in pool, seed badge in leaderboard
- [x] `evolution/docs/cost_optimization.md` — per-iteration budget enforcement, iteration budget tracker
- [x] `evolution/docs/data_model.md` — iterationConfigs in strategy config JSONB, generation field semantics
- [x] `evolution/docs/strategies_and_experiments.md` — multi-iteration strategy config, new wizard, per-iteration fields
- [x] `evolution/docs/visualization.md` — per-iteration dashboard views, new strategy variants tab
- [x] `evolution/docs/agents/overview.md` — per-iteration agent type selection
- [x] `evolution/docs/reference.md` — updated file index, new pages, new actions
- [x] `evolution/docs/metrics.md` — per-iteration metrics in snapshots
- [x] `docs/feature_deep_dives/evolution_metrics.md` — per-iteration cost display
- [x] `docs/feature_deep_dives/multi_iteration_strategies.md` — new deep dive (created during init)
