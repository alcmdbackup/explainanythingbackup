...
export const getStrategyAccuracyAction = adminAction('getStrategyAccuracyAction', async (ctx: AdminContext) => {
  const { supabase } = ctx;
  ...
  if (!runs || runs.length === 0) return [] as StrategyAccuracyStats[];
