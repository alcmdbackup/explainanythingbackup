...
/** Options for cleanupEvolutionData. All fields optional — only provided IDs are cleaned. */
...
export async function cleanupEvolutionData(
  ...
  try {
    ...
    // Delete strategy configs (after runs that reference them)
    if (strategyIds.length > 0) {
      await supabase.from('evolution_strategy_configs').delete().in('id', strategyIds);
    }

    // Delete arena topics / prompts (after runs that reference them)
    if (promptIds.length > 0) {
      await supabase.from('evolution_arena_topics').delete().in('id', promptIds);
    }
  } catch (error) {
