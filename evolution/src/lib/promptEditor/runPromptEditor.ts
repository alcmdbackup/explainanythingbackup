// Playground dispatcher: pre-flight per-run cost cap, then runs every config in parallel via
...
import { runPlaygroundConfig } from './runPlaygroundConfig';
import type { PlaygroundRunInput, PlaygroundRunResult, PlaygroundConfig, RewriteUnit } from './types';
...
export class PlaygroundCostCapError extends Error {
  constructor(public readonly estimatedUsd: number, public readonly capUsd: number) {
    ...
    this.name = 'PlaygroundCostCapError';
  }
}
...
export function estimatePlaygroundConfigCost(
  unit: RewriteUnit,
  sourceText: string,
...
export function estimatePlaygroundRunCost(input: PlaygroundRunInput): number {
  return input.configs.reduce(
    (sum, c) => sum + estimatePlaygroundConfigCost(input.unit, input.sourceText, c, input.title ?? ''),
    0,
  );
}

/**
 * Run all configs. Throws PlaygroundCostCapError if the pre-flight estimate exceeds the cap;
...
export async function runPlayground(input: PlaygroundRunInput): Promise<PlaygroundRunResult> {
  const estimate = estimatePlaygroundRunCost(input);
  if (estimate > PLAYGROUND_PER_RUN_CAP_USD) {
    throw new PlaygroundCostCapError(estimate, PLAYGROUND_PER_RUN_CAP_USD);
  }

  const settled = await Promise.allSettled(
    input.configs.map((c) => runPlaygroundConfig(input.unit, input.sourceText, c, input.title ?? '')),
  );
