// Unit tests for runPlayground: pre-flight cost-cap rejection, parallel dispatch, and
// per-config failure isolation under Promise.allSettled.

import {
  runPlayground,
  estimatePlaygroundRunCost,
  PlaygroundCostCapError,
  PLAYGROUND_PER_RUN_CAP_USD,
} from './runPlayground';
import type { PlaygroundRunInput } from './types';
...
describe('estimatePlaygroundRunCost', () => {
  it('sums a positive estimate across configs', () => {
    ...
    expect(estimatePlaygroundRunCost(input)).toBeGreaterThan(0);
  });
});

describe('runPlayground', () => {
  it('throws PlaygroundCostCapError when the estimate exceeds the cap', async () => {
    ...
    await expect(runPlayground(input)).rejects.toBeInstanceOf(PlaygroundCostCapError);
  ...
  it('runs all configs in parallel and aggregates cost', async () => {
    ...
    const input: PlaygroundRunInput = { unit: 'article', sourceText: 'short src', configs: [cfg('A'), cfg('B'), cfg('C')] };
    const res = await runPlayground(input);
  ...
  it('isolates one failing config (rejection does not block siblings)', async () => {
    ...
    const input: PlaygroundRunInput = { unit: 'article', sourceText: 'src', configs: [cfg('A'), cfg('B')] };
    const res = await runPlayground(input);
  ...
  it('keeps the cap constant', () => {
    expect(PLAYGROUND_PER_RUN_CAP_USD).toBe(0.5);
  });
});
