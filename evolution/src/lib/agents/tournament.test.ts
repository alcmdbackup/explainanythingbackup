...
// ─── Logistic CDF pairing tests ──────────────────────────────────

describe('swissPairing logistic CDF outcome uncertainty', () => {
  it('equal ratings produce maximum outcome uncertainty', () => {
    const state = makeState(2);
    const ratings = new Map<string, Rating>();
    ratings.set('v-0', { mu: 25, sigma: 4 });
    ratings.set('v-1', { mu: 25, sigma: 4 });
    const pairs = swissPairing(state.pool, ratings, new Set(), 4);
    expect(pairs).toHaveLength(1);
  });

  it('large gap produces near-zero outcome uncertainty (low priority)', () => {
    const state = makeState(4);
    const ratings = new Map<string, Rating>();
    // v-0 and v-1 are very close, v-2 and v-3 are very far apart
    ratings.set('v-0', { mu: 25, sigma: 4 });
    ratings.set('v-1', { mu: 25.1, sigma: 4 });
    ratings.set('v-2', { mu: 50, sigma: 4 });
    ratings.set('v-3', { mu: 5, sigma: 4 });
    const pairs = swissPairing(state.pool, ratings, new Set(), 4);
    expect(pairs).toHaveLength(2);
    // Top pair should be the close-rated pair (v-0/v-1) since they have higher outcome uncertainty
    const topPairIds = [pairs[0][0].id, pairs[0][1].id].sort();
    expect(topPairIds).toEqual(['v-0', 'v-1']);
  });

  it('symmetry: swapping A and B produces same outcome uncertainty', () => {
    // This is an implicit test: logistic CDF is symmetric around 0.5
    // so |2*pWin - 1| is the same regardless of which variant is A or B
    const state = makeState(4);
    const ratings = new Map<string, Rating>();
    ratings.set('v-0', { mu: 30, sigma: 4 });
