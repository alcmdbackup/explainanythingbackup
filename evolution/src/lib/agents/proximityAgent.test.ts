...
describe('semantic embedding blend', () => {
  /** Mock embedText that returns normalized vectors based on a simple hash. */
  function mockEmbedText(text: string): Promise<number[]> {
    // Simple mock: use char codes to produce a deterministic 4-dim vector
    const vec = [0, 0, 0, 0];
  ...
  it('blends semantic and lexical similarity when embedText provided', async () => {
    const agent = new ProximityAgent({ testMode: true });
    const state = new PipelineStateImpl('original');
    state.addToPool(makeVariation('v1', 'The cat sat on the mat'));
    state.startNewIteration();
    state.addToPool(makeVariation('v2', 'A dog ran through the park'));
  ...
  it('produces different similarity than lexical-only when embedText provided', async () => {
    // Run once without embedText (lexical-only)
    const agent1 = new ProximityAgent({ testMode: true });
    const state1 = new PipelineStateImpl('original');
    state1.addToPool(makeVariation('v1', 'The cat sat on the mat'));
    ...
    const state2 = new PipelineStateImpl('original');
    state2.addToPool(makeVariation('v1', 'The cat sat on the mat'));
    state2.startNewIteration();
  ...
  it('falls back to lexical-only when embedText throws', async () => {
    const agent = new ProximityAgent({ testMode: true });
    const state = new PipelineStateImpl('original');
    state.addToPool(makeVariation('v1', 'Text about cats'));
    state.startNewIteration();
    state.addToPool(makeVariation('v2', 'Text about dogs'));
  ...
  it('falls back to lexical-only when embedText is undefined', async () => {
    ...
    const ctx = makeCtx(state);
    // No embedText set — should use lexical-only
    const result = await agent.execute(ctx);
  ...
  it('caches semantic embeddings across iterations', async () => {
    ...
    const trackingEmbed = (text: string) => {
      embedCallCount++;
    ...
    const ctx = makeCtx(state);
    ctx.embedText = trackingEmbed;

    // First execution: embeds v1 + v2
    await agent.execute(ctx);
    const firstCallCount = embedCallCount;

    // Second execution (no new entrants, so should be no-op)
    state.startNewIteration();
    state.addToPool(makeVariation('v3', 'Text C'));
    await agent.execute(ctx);

    // Only v3 should be newly embedded (v1 and v2 cached)
    expect(embedCallCount).toBe(firstCallCount + 1);
  });
});
