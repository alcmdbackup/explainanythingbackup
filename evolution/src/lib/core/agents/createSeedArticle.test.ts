...
jest.mock('../../pipeline/setup/generateSeedArticle', () => ({
  ...
  buildArticlePrompt: jest.fn((title: string) => `mock article prompt for ${title}`),
}));
...
describe('CreateSeedArticleAgent', () => {
  ...
  it('continues (non-fatal) and records formatValid=false when format validation fails', async () => {
    ...
    const agent = new CreateSeedArticleAgent();
    const output = await agent.execute(makeInput(), makeCtx());

    // Format failure is non-fatal — variant is still created and ranked
    expect(output.result.variant).not.toBeNull();
    expect(output.detail.generation.formatValid).toBe(false);
  });
