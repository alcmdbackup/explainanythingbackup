...
import { generateTitle } from '../../pipeline/setup/generateSeedArticle';
...
> {
  ...
  async execute(
    ...
    try {
      ...
      return { result: { variant: null, status: 'generation_failed', surfaced: false, matches: [] }, detail };
    }
    ...
    if (!fmt.valid) {
      ctx.logger.warn('createSeedArticle: format validation failed', {
    ...
    // Step 2 + 3: rank against arena pool, apply surface/discard decision.
