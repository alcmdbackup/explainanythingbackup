...
export interface MergeRatingsInput {
  iterationType: 'generate' | 'reflect_and_generate' | 'iterative_editing' | 'swiss' | 'debate_and_generate';
...
> {
  ...
  async execute(
    ...
    for (let i = 0; i < allMatches.length; i++) {
      ...
      // DB columns remain named mu/sigma (not renamed due to CI safety check).
      // Application layer exposes these as elo/uncertainty via dbToRating/ratingToDb,
      // but the raw column values are still openskill mu/sigma derived via ratingToDb.
