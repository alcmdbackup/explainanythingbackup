...
describe('strategyConfigSchema', () => {
  ...
  // investigate_iterative_editing_runs_stage_20260623 dropped both fields.
  // Existing strategy JSONB rows that still carry them are silent-tolerated:
  // Zod's default object behavior strips unknown keys at parse time.
  describe('editingProposerSoftCap + disableApproverFiltering — silent-tolerate on load', () => {
    it('strips a legacy iterationConfigs payload that still carries editingProposerSoftCap and disableApproverFiltering (no parse error)', () => {
      const result = strategyConfigSchema.parse({
        ...validBase,
        iterationConfigs: [
          { agentType: 'generate', budgetPercent: 34 },
          {
            agentType: 'iterative_editing_rewrite',
            budgetPercent: 66,
            // Legacy fields. Zod default behavior on z.object — strip unknown keys.
            editingProposerSoftCap: 8,
            disableApproverFiltering: true,
          },
        ],
      });
      const iter = result.iterationConfigs[1]! as Record<string, unknown>;
      expect(iter.editingProposerSoftCap).toBeUndefined();
      expect(iter.disableApproverFiltering).toBeUndefined();
    });

    it('strips legacy fields on Mode A (iterative_editing) too — no per-agent-type rejection', () => {
      const result = strategyConfigSchema.parse({
        ...validBase,
        iterationConfigs: [
          { agentType: 'generate', budgetPercent: 34 },
          {
            ...
            budgetPercent: 66,
            editingProposerSoftCap: 3,
            disableApproverFiltering: false,
          },
        ],
      });
      const iter = result.iterationConfigs[1]! as Record<string, unknown>;
