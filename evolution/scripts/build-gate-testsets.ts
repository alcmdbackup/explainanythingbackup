...
async function main(): Promise<void> {
  ...
  const a = await getOrCreateTestSet(db as never, bank, {
    name: 'gate-article-lg60', strategy: 'manual', seed: 1, sizeArticle: articleLabels.length, sizeParagraph: 0, manualLabels: articleLabels,
  });
  ...
  const p = await getOrCreateTestSet(db as never, bank, {
    name: 'gate-paragraph-lg60', strategy: 'manual', seed: 1, sizeArticle: 0, sizeParagraph: paragraphLabels.length, manualLabels: paragraphLabels,
  });
